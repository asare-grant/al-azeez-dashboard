...
Generated: 2026-08-18T13:52:41.508Z

## Executive Summary

- Files scanned: **733**
- Total findings: **816**
- Migration risk score: **3720**
- Good RBAC signals: **115**
...
| LEGACY_ROLE | 143 |
| CENTRALIZED | 115 |
| LEGACY_ROLE_GUARD | 81 |
| ROLE_SCOPE_REVIEW | 76 |
| AUTH_ONLY_REVIEW | 61 |
| CLAIMS | 39 |
...
```ts
     76 |   currentState: CurrentState,
     77 |   data: SubjectSchema,
     78 | ) => {
     79 |   try {
>    80 |     await prisma.subject.create({
     81 |       data: {
     82 |         name: data.name,
     83 |         teachers: {
     84 |           connect: data.teachers.map((teacherId) => ({ id: teacherId })),
```
...
```ts
     ...
     99 |   data: SubjectSchema,
    100 | ) => {
    101 |   try {
>   102 |     await prisma.subject.update({
    103 |       where: {
    104 |         id: data.id,
    105 |       },
    106 |       data: {
...
```ts
    124 |   data: FormData,
    125 | ) => {
    126 |   const id = data.get("id") as string;
    127 |   try {
>   128 |     await prisma.subject.delete({
    129 |       where: {
    130 |         id: parseInt(id),
    131 |       },
    132 |     });
...
```ts
    143 |   currentState: CurrentState,
    144 |   data: ClassSchema,
    145 | ) => {
    146 |   try {
...
```ts
    163 |   if (!data.id) {
    164 |     return { success: false, error: true };
    165 |   }
    166 |   try {
>   167 |     await prisma.class.update({
    168 |       where: {
    169 |         id: data.id,
    170 |       },
    171 |       data,
...
```ts
    184 |   data: FormData,
    185 | ) => {
    186 |   const id = data.get("id") as string;
    187 |   try {
>   188 |     await prisma.class.delete({
    189 |       where: {
    190 |         id: parseInt(id),
    191 |       },
    192 |     });
...
```ts
    213 |       lastName: data.surname,
    214 |       publicMetadata: { role: "teacher" },
    215 |     });
    216 | 
>   217 |     await prisma.teacher.create({
    218 |       data: {
    219 |         id: user.id,
...
```ts
    259 |       firstName: data.name,
    260 |       lastName: data.surname,
    261 |     });
    262 | 
>   263 |     await prisma.teacher.update({
    264 |       where: {
    265 |         id: data.id,
    266 |       },
    267 |       data: {
...
```ts
    300 |     const client = await clerkClient();
    301 | 
    302 |     await client.users.deleteUser(id);
    303 | 
>   304 |     await prisma.teacher.delete({
    305 |       where: {
    306 |         id: id,
    307 |       },
    308 |     });
...
```ts
    339 |       lastName: data.surname,
    340 |       publicMetadata: { role: "student" },
    341 |     });
    342 | 
>   343 |     await prisma.student.create({
    344 |       data: {
    345 |         id: user.id,
    346 |         username: data.username,
    347 |         name: data.name,
```
...
```ts
    385 |       firstName: data.name,
    386 |       lastName: data.surname,
    387 |     });
    388 | 
>   389 |     await prisma.student.update({
    390 |       where: {
    391 |         id: data.id,
    392 |       },
    393 |       data: {
...
```ts
    426 |     const client = await clerkClient();
    427 | 
    428 |     await client.users.deleteUser(id);
    429 | 
>   430 |     await prisma.student.delete({
    431 |       where: {
    432 |         id: id,
    433 |       },
    434 |     });
...
```ts
    459 |       publicMetadata: { role: "parent" },
    460 |     });
    461 | 
    462 |     // Create parent in Prisma
>   463 |     await prisma.parent.create({
    464 |       data: {
    465 |         id: user.id, // Clerk user id as parent id
    466 |         username: data.username,
    467 |         name: data.name,
```
...
```ts
    501 |       lastName: data.surname,
    502 |     });
    503 | 
    504 |     // Update Prisma parent record
>   505 |     await prisma.parent.update({
    506 |       where: { id: data.id },
    507 |       data: {
    508 |         username: data.username,
    509 |         name: data.name,
```
...
```ts
    541 |   try {
    542 |     const client = await clerkClient();
    543 | 
    544 |     // 1️⃣ Delete associated students first (if any)
>   545 |     await prisma.student.deleteMany({
    546 |       where: { parentId: id },
    547 |     });
    548 | 
    549 |     // 2️⃣ Delete parent from Prisma
```
...
```ts
    ...
    548 | 
    549 |     // 2️⃣ Delete parent from Prisma
>   550 |     await prisma.parent.delete({
    551 |       where: { id },
    552 |     });
    553 | 
    554 |     // 3️⃣ Delete parent from Clerk
```
...
```ts
    564 |       console.warn(
    565 |         "⚠️ Parent not found in Clerk. Continuing with Prisma deletion.",
    566 |       );
    567 |       try {
>   568 |         await prisma.parent.delete({ where: { id } });
    569 |         return { success: true, error: false };
    570 |       } catch (prismaErr) {
    571 |         console.error("❌ Error deleting parent from Prisma:", prismaErr);
    572 |       }
```
...
```ts
   ...
   1012 |     /* ---------------------------------------------------------------------- */
   1013 | 
>  1014 |     await prisma.lesson.create({
   1015 |       data: {
   1016 |         name: data.name,
   1017 | 
   1018 |         day: data.day,
...
```ts
   1119 |     const startTime = new Date(`1970-01-01T${data.startTime}:00Z`);
   1120 | 
   1121 |     const endTime = new Date(`1970-01-01T${data.endTime}:00Z`);
   1122 | 
>  1123 |     await prisma.lesson.update({
   1124 |       where: {
   1125 |         id: data.id,
   1126 |       },
   1127 | 
...
```ts
   ...
   1214 |     }
   1215 | 
>  1216 |     await prisma.lesson.delete({
   1217 |       where: {
   1218 |         id: lessonId,
   1219 |       },
   1220 |     });
...
```ts
   ...
   1353 |     /* -------------------------------------------------------------------- */
   1354 | 
>  1355 |     await prisma.assignment.create({
   1356 |       data: {
   1357 |         title: data.title,
   1358 | 
   1359 |         startDate: data.startDate,
```
...
```ts
   ...
   1529 |     /* -------------------------------------------------------------------- */
   1530 | 
>  1531 |     await prisma.assignment.update({
   1532 |       where: {
   1533 |         id: data.id,
   1534 |       },
   1535 | 
...
```ts
   ...
   1653 |     /* -------------------------------------------------------------------- */
   1654 | 
>  1655 |     await prisma.assignment.delete({
   1656 |       where: {
   1657 |         id: assignmentId,
   1658 |       },
   1659 |     });
```
...
```ts
   2062 |    * Keep NotificationEvent as historical/audit data,
   2063 |    * but hide deliveries that describe an obsolete
   2064 |    * event schedule.
   2065 |    */
>  2066 |   await tx.notification.updateMany({
   2067 |     where: {
   2068 |       archivedAt: null,
   2069 | 
   2070 |       event: {
```
...
```ts
   2160 |     /* ---------------------------------------------------------------------- */
   2161 | 
   2162 |     const event = await prisma.$transaction(
   2163 |       async (tx) => {
>  2164 |         const created = await tx.event.create({
   2165 |           data: {
   2166 |             title: values.title,
   2167 | 
   2168 |             description: values.description,
```
...
```ts
   ...
   2397 |         /* -------------------------------------------------------------- */
   2398 | 
>  2399 |         const updated = await tx.event.update({
   2400 |           where: {
   2401 |             id: eventId,
   2402 |           },
   2403 | 
...
```ts
   ...
   2762 |     /* ---------------------------------------------------------------------- */
   2763 | 
   2764 |     const announcement =
>  2765 |       await prisma.announcement.create({
   2766 |         data: {
   2767 |           title:
   2768 |             data.title,
   2769 | 
...
```ts
   2955 |           "The announcement could not be found.",
   2956 |       };
   2957 |     }
   2958 | 
>  2959 |     await prisma.announcement.update({
   2960 |       where: {
   2961 |         id:
   2962 |           announcementId,
   2963 |       },
...
```ts
   3100 |           "The announcement could not be found.",
   3101 |       };
   3102 |     }
   3103 | 
>  3104 |     await prisma.announcement.delete({
   3105 |       where: {
   3106 |         id:
   3107 |           announcementId,
   3108 |       },
...
```ts
   3181 |     await requireFinancePermission(
   3182 |       "finance.structure.manage",
   3183 |     );
   3184 | 
>  3185 |     await prisma.feeCategory.create({
   3186 |       data: {
   3187 |         name:
   3188 |           data.name,
   3189 |       },
...
```ts
   3224 |     await requireFinancePermission(
   3225 |       "finance.structure.manage",
   3226 |     );
   3227 | 
>  3228 |     await prisma.feeCategory.update({
   3229 |       where: { id: data.id },
   3230 |       data: { name: data.name },
   3231 |     });
   3232 | 
```
...
```ts
   3245 |     await requireFinancePermission(
   3246 |       "finance.structure.manage",
   3247 |     );
   3248 | 
>  3249 |     await prisma.feeCategory.delete({ where: { id: parseInt(id) } });
   3250 |     return { success: true, error: false };
   3251 |   } catch (err) {
   3252 |     console.log("DELETE FEE CATEGORY ERROR:", err);
   3253 |     return { success: false, error: true };
```
...
```ts
   3259 |     await requireFinancePermission(
   3260 |       "finance.structure.manage",
   3261 |     );
   3262 | 
>  3263 |     await prisma.feeType.create({
   3264 |       data: {
   3265 |         name: data.name,
   3266 |         categoryId: data.categoryId,
   3267 |       },
```
...
```ts
   3281 |     await requireFinancePermission(
   3282 |       "finance.structure.manage",
   3283 |     );
   3284 | 
>  3285 |     await prisma.feeType.update({
   3286 |       where: { id: data.id },
   3287 |       data: {
   3288 |         name: data.name,
   3289 |         categoryId: data.categoryId,
```
...
```ts
   3305 |     await requireFinancePermission(
   3306 |       "finance.structure.manage",
   3307 |     );
   3308 | 
>  3309 |     await prisma.feeType.delete({ where: { id: parseInt(id) } });
   3310 |     return { success: true, error: false };
   3311 |   } catch (err) {
   3312 |     console.log("DELETE FEE TYPE ERROR:", err);
   3313 |     return { success: false, error: true };
```
...
```ts
   3322 |     await requireFinancePermission(
   3323 |       "finance.structure.manage",
   3324 |     );
   3325 | 
>  3326 |     await prisma.feeStructure.create({
   3327 |       data: {
   3328 |         amount: data.amount,
   3329 |         studentType: data.studentType,
   3330 |         boardingType: data.boardingType,
```
...
```ts
   3351 |     await requireFinancePermission(
   3352 |       "finance.structure.manage",
   3353 |     );
   3354 | 
>  3355 |     await prisma.feeStructure.update({
   3356 |       where: { id: data.id },
   3357 |       data: {
   3358 |         amount: data.amount,
   3359 |         studentType: data.studentType,
```
...
```ts
   3379 |     await requireFinancePermission(
   3380 |       "finance.structure.manage",
   3381 |     );
   3382 | 
>  3383 |     await prisma.feeStructure.delete({ where: { id: parseInt(id) } });
   3384 |     return { success: true, error: false };
   3385 |   } catch (err) {
   3386 |     console.log("DELETE FEE STRUCTURE ERROR:", err);
   3387 |     return { success: false, error: true };
```
...
```ts
   3396 |     await requireFinancePermission(
   3397 |       "finance.invoices.manage",
   3398 |     );
   3399 | 
>  3400 |     await prisma.feeMaster.create({
   3401 |       data: {
   3402 |         studentId: data.studentId,
   3403 |         term: data.term,
   3404 |         academicYear: data.academicYear,
```
...
```ts
   3424 |     await requireFinancePermission(
   3425 |       "finance.invoices.manage",
   3426 |     );
   3427 | 
>  3428 |     await prisma.feeMaster.update({
   3429 |       where: { id: data.id },
   3430 |       data: {
   3431 |         studentId: data.studentId,
   3432 |         term: data.term,
```
...
```ts
   3451 |     await requireFinancePermission(
   3452 |       "finance.invoices.manage",
   3453 |     );
   3454 | 
>  3455 |     await prisma.feeMaster.delete({ where: { id: parseInt(id) } });
   3456 |     return { success: true, error: false };
   3457 |   } catch (err) {
   3458 |     console.log("DELETE FEE MASTER ERROR:", err);
   3459 |     return { success: false, error: true };
```
...
```ts
   3465 |     await requireFinancePermission(
   3466 |       "finance.invoices.manage",
   3467 |     );
   3468 | 
>  3469 |     await prisma.fee.create({
   3470 |       data: {
   3471 |         masterId: data.masterId,
   3472 |         structureId: data.structureId,
   3473 |         amount: data.amount,
```
...
```ts
   3488 |     await requireFinancePermission(
   3489 |       "finance.invoices.manage",
   3490 |     );
   3491 | 
>  3492 |     await prisma.fee.update({
   3493 |       where: { id: data.id },
   3494 |       data: {
...
```ts
   3513 |     await requireFinancePermission(
   3514 |       "finance.invoices.manage",
   3515 |     );
   3516 | 
>  3517 |     await prisma.fee.delete({ where: { id: parseInt(id) } });
   3518 |     return { success: true, error: false };
   3519 |   } catch (err) {
   3520 |     console.log("DELETE FEE ERROR:", err);
   3521 |     return { success: false, error: true };
```
...
```ts
   ...
   3654 |         }
   3655 | 
>  3656 |         const payment = await tx.feePayment.create({
   3657 |           data: {
   3658 |             masterId: data.masterId,
   3659 | 
   3660 |             amount: data.amount,
```
...
```ts
   ...
   3831 |         /* ------------------------------------------------------------------ */
   3832 | 
>  3833 |         await tx.feePayment.update({
   3834 |           where: {
   3835 |             id:
   3836 |               data.id!,
   3837 |           },
...
```ts
   4014 |     await prisma.$transaction(
   4015 |       async (
   4016 |         tx,
   4017 |       ) => {
>  4018 |         await tx.feePayment.delete({
   4019 |           where: {
   4020 |             id:
   4021 |               payment.id,
   4022 |           },
...
```ts
   ...
   4177 |   /* ------------------------------------------------------------------------ */
   4178 | 
>  4179 |   return prisma.feeMaster.create({
   4180 |     data: {
   4181 |       studentId,
   4182 | 
   4183 |       term,
...
```ts
   4216 |   currentState: CurrentState,
   4217 |   data: AttendanceSchema,
   4218 | ) => {
   4219 |   try {
>  4220 |     await prisma.attendance.create({
   4221 |       data: {
   4222 |         date: data.date,
   4223 |         present: data.present,
   4224 |         day: data.day,
```
...
```ts
   4239 | ) => {
   4240 |   if (!data.id) return { success: false, error: true };
   4241 | 
   4242 |   try {
>  4243 |     await prisma.attendance.update({
   4244 |       where: { id: data.id },
   4245 |       data: {
   4246 |         date: data.date,
   4247 |         present: data.present,
```
...
```ts
   4460 |          * The school has one current active
   4461 |          * term across the application.
   4462 |          */
   4463 |         if (data.isActive) {
>  4464 |           await tx.schoolTerm.updateMany({
   4465 |             where: {
   4466 |               isActive: true,
   4467 | 
   4468 |               ...(data.id
...
```ts
   4525 |           const termDateRangeChanged =
   4526 |             existing.startDate.getTime() !== startDate.getTime() ||
   4527 |             existing.endDate.getTime() !== endDate.getTime();
   4528 | 
>  4529 |           const updated = await tx.schoolTerm.update({
   4530 |             where: {
   4531 |               id: data.id,
   4532 |             },
   4533 | 
...
```ts
   ...
   4625 |         /* ------------------------------------------------------------ */
   4626 | 
>  4627 |         const created = await tx.schoolTerm.create({
   4628 |           data: {
   4629 |             academicYearId: data.academicYearId,
   4630 | 
   4631 |             name: data.name,
```
...
```ts
   4756 |        * Only one academic year should
   4757 |        * be active at a time.
   4758 |        */
   4759 |       if (data.isActive) {
>  4760 |         await tx.schoolAcademicYear.updateMany({
   4761 |           where: {
   4762 |             isActive: true,
   4763 |           },
   4764 | 
...
```ts
   ...
   4769 |       }
   4770 | 
>  4771 |       return tx.schoolAcademicYear.create({
   4772 |         data: {
   4773 |           name,
   4774 | 
   4775 |           startDate: data.startDate,
```
...
```ts
   4873 |         throw new Error("The academic year could not be found.");
   4874 |       }
   4875 | 
   4876 |       if (data.isActive) {
>  4877 |         await tx.schoolAcademicYear.updateMany({
   4878 |           where: {
   4879 |             isActive: true,
   4880 | 
   4881 |             NOT: {
...
```ts
   ...
   4890 |       }
   4891 | 
>  4892 |       return tx.schoolAcademicYear.update({
   4893 |         where: {
   4894 |           id: data.id,
   4895 |         },
   4896 | 
...
`src/lib/actions.ts:3573`

```ts
   3569 |   const userId = accessActor.actor.id;
   3570 | 
   3571 |   const actorRole =
   3572 |     grantingAssignment?.role.key?.trim().toLowerCase() ??
>  3573 |     accessActor.actor.legacyRole?.trim().toLowerCase() ??
   3574 |     null;
   3575 | 
   3576 |   const actorName =
   3577 |     accessActor.actor.displayName?.trim() ||
```
...
`src/lib/actions.ts:4321`

```ts
   4317 |     const userId = accessActor.actor.id;
   4318 | 
   4319 |     const actorRole =
   4320 |       grantingAssignment?.role.key?.trim().toLowerCase() ??
>  4321 |       accessActor.actor.legacyRole?.trim().toLowerCase() ??
   4322 |       null;
   4323 | 
   4324 |     const actorName =
   4325 |       accessActor.actor.displayName?.trim() ||
```
