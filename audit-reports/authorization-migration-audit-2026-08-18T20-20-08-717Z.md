...
Generated: 2026-08-18T20:20:08.737Z

## Executive Summary

- Files scanned: **733**
- Total findings: **782**
- Migration risk score: **3462**
- Good RBAC signals: **136**
...
```ts
    ...
    492 |     /* ---------------------------------------------------------------------- */
    493 | 
>   494 |     await prisma.teacher.update({
    495 |       where: {
    496 |         id: data.id,
    497 |       },
    498 | 
...
```ts
    ...
    711 | 
    712 |     try {
>   713 |       await prisma.student.create({
    714 |         data: {
    715 |           id: user.id,
    716 | 
    717 |           username: data.username,
```
...
```ts
    ...
    888 |     /* ---------------------------------------------------------------------- */
    889 | 
>   890 |     await prisma.student.update({
    891 |       where: {
    892 |         id: data.id,
    893 |       },
    894 | 
...
```ts
   1057 |       publicMetadata: { role: "parent" },
   1058 |     });
   1059 | 
   1060 |     // Create parent in Prisma
>  1061 |     await prisma.parent.create({
   1062 |       data: {
   1063 |         id: user.id, // Clerk user id as parent id
   1064 |         username: data.username,
   1065 |         name: data.name,
```
...
```ts
   1099 |       lastName: data.surname,
   1100 |     });
   1101 | 
   1102 |     // Update Prisma parent record
>  1103 |     await prisma.parent.update({
   1104 |       where: { id: data.id },
   1105 |       data: {
   1106 |         username: data.username,
   1107 |         name: data.name,
```
...
```ts
   1139 |   try {
   1140 |     const client = await clerkClient();
   1141 | 
   1142 |     // 1️⃣ Delete associated students first (if any)
>  1143 |     await prisma.student.deleteMany({
   1144 |       where: { parentId: id },
   1145 |     });
   1146 | 
   1147 |     // 2️⃣ Delete parent from Prisma
```
...
```ts
   ...
   1146 | 
   1147 |     // 2️⃣ Delete parent from Prisma
>  1148 |     await prisma.parent.delete({
   1149 |       where: { id },
   1150 |     });
   1151 | 
   1152 |     // 3️⃣ Delete parent from Clerk
```
...
```ts
   1162 |       console.warn(
   1163 |         "⚠️ Parent not found in Clerk. Continuing with Prisma deletion.",
   1164 |       );
   1165 |       try {
>  1166 |         await prisma.parent.delete({ where: { id } });
   1167 |         return { success: true, error: false };
   1168 |       } catch (prismaErr) {
   1169 |         console.error("❌ Error deleting parent from Prisma:", prismaErr);
   1170 |       }
```
...
```ts
   ...
   1259 |     /* ---------------------------------------------------------------------- */
   1260 | 
>  1261 |     await prisma.exam.create({
   1262 |       data: {
   1263 |         title: data.title,
   1264 | 
   1265 |         startTime: data.startTime,
```
...
```ts
   ...
   1610 |     /* ---------------------------------------------------------------------- */
   1611 | 
>  1612 |     await prisma.lesson.create({
   1613 |       data: {
   1614 |         name: data.name,
   1615 | 
   1616 |         day: data.day,
...
```ts
   1717 |     const startTime = new Date(`1970-01-01T${data.startTime}:00Z`);
   1718 | 
   1719 |     const endTime = new Date(`1970-01-01T${data.endTime}:00Z`);
   1720 | 
>  1721 |     await prisma.lesson.update({
   1722 |       where: {
   1723 |         id: data.id,
   1724 |       },
   1725 | 
...
```ts
   ...
   1812 |     }
   1813 | 
>  1814 |     await prisma.lesson.delete({
   1815 |       where: {
   1816 |         id: lessonId,
   1817 |       },
   1818 |     });
...
```ts
   ...
   1951 |     /* -------------------------------------------------------------------- */
   1952 | 
>  1953 |     await prisma.assignment.create({
   1954 |       data: {
   1955 |         title: data.title,
   1956 | 
   1957 |         startDate: data.startDate,
```
...
```ts
   ...
   2127 |     /* -------------------------------------------------------------------- */
   2128 | 
>  2129 |     await prisma.assignment.update({
   2130 |       where: {
   2131 |         id: data.id,
   2132 |       },
   2133 | 
...
```ts
   ...
   2251 |     /* -------------------------------------------------------------------- */
   2252 | 
>  2253 |     await prisma.assignment.delete({
   2254 |       where: {
   2255 |         id: assignmentId,
   2256 |       },
   2257 |     });
```
...
```ts
   2660 |    * Keep NotificationEvent as historical/audit data,
   2661 |    * but hide deliveries that describe an obsolete
   2662 |    * event schedule.
   2663 |    */
>  2664 |   await tx.notification.updateMany({
   2665 |     where: {
   2666 |       archivedAt: null,
   2667 | 
   2668 |       event: {
```
...
```ts
   2758 |     /* ---------------------------------------------------------------------- */
   2759 | 
   2760 |     const event = await prisma.$transaction(
   2761 |       async (tx) => {
>  2762 |         const created = await tx.event.create({
   2763 |           data: {
   2764 |             title: values.title,
   2765 | 
   2766 |             description: values.description,
```
...
```ts
   ...
   2995 |         /* -------------------------------------------------------------- */
   2996 | 
>  2997 |         const updated = await tx.event.update({
   2998 |           where: {
   2999 |             id: eventId,
   3000 |           },
   3001 | 
...
```ts
   ...
   3348 |     /* ---------------------------------------------------------------------- */
   3349 | 
>  3350 |     const announcement = await prisma.announcement.create({
   3351 |       data: {
   3352 |         title: data.title,
   3353 | 
   3354 |         description: data.description,
```
...
```ts
   3470 |         message: "The announcement could not be found.",
   3471 |       };
   3472 |     }
   3473 | 
>  3474 |     await prisma.announcement.update({
   3475 |       where: {
   3476 |         id: announcementId,
   3477 |       },
   3478 | 
```
...
```ts
   3561 |         message: "The announcement could not be found.",
   3562 |       };
   3563 |     }
   3564 | 
>  3565 |     await prisma.announcement.delete({
   3566 |       where: {
   3567 |         id: announcementId,
   3568 |       },
   3569 |     });
```
...
```ts
   3615 | ) => {
   3616 |   try {
   3617 |     await requireFinancePermission("finance.structure.manage");
   3618 | 
>  3619 |     await prisma.feeCategory.create({
   3620 |       data: {
   3621 |         name: data.name,
   3622 |       },
   3623 |     });
...
```ts
   3646 | 
   3647 |   try {
   3648 |     await requireFinancePermission("finance.structure.manage");
   3649 | 
>  3650 |     await prisma.feeCategory.update({
   3651 |       where: { id: data.id },
   3652 |       data: { name: data.name },
   3653 |     });
   3654 | 
```
...
```ts
   3665 | 
   3666 |   try {
   3667 |     await requireFinancePermission("finance.structure.manage");
   3668 | 
>  3669 |     await prisma.feeCategory.delete({ where: { id: parseInt(id) } });
   3670 |     return { success: true, error: false };
   3671 |   } catch (err) {
   3672 |     console.log("DELETE FEE CATEGORY ERROR:", err);
   3673 |     return { success: false, error: true };
```
...
```ts
   3677 | export const createFeeType = async (currentState: any, data: FeeTypeSchema) => {
   3678 |   try {
   3679 |     await requireFinancePermission("finance.structure.manage");
   3680 | 
>  3681 |     await prisma.feeType.create({
   3682 |       data: {
   3683 |         name: data.name,
   3684 |         categoryId: data.categoryId,
   3685 |       },
```
...
```ts
   3697 | 
   3698 |   try {
   3699 |     await requireFinancePermission("finance.structure.manage");
   3700 | 
...
```ts
   3719 | 
   3720 |   try {
   3721 |     await requireFinancePermission("finance.structure.manage");
   3722 | 
>  3723 |     await prisma.feeType.delete({ where: { id: parseInt(id) } });
   3724 |     return { success: true, error: false };
   3725 |   } catch (err) {
   3726 |     console.log("DELETE FEE TYPE ERROR:", err);
   3727 |     return { success: false, error: true };
```
...
```ts
   3734 | ) => {
   3735 |   try {
   3736 |     await requireFinancePermission("finance.structure.manage");
   3737 | 
>  3738 |     await prisma.feeStructure.create({
   3739 |       data: {
   3740 |         amount: data.amount,
   3741 |         studentType: data.studentType,
   3742 |         boardingType: data.boardingType,
```
...
```ts
   3761 | 
   3762 |   try {
   3763 |     await requireFinancePermission("finance.structure.manage");
   3764 | 
>  3765 |     await prisma.feeStructure.update({
   3766 |       where: { id: data.id },
   3767 |       data: {
   3768 |         amount: data.amount,
   3769 |         studentType: data.studentType,
```
...
```ts
   3787 | 
   3788 |   try {
   3789 |     await requireFinancePermission("finance.structure.manage");
   3790 | 
>  3791 |     await prisma.feeStructure.delete({ where: { id: parseInt(id) } });
   3792 |     return { success: true, error: false };
   3793 |   } catch (err) {
   3794 |     console.log("DELETE FEE STRUCTURE ERROR:", err);
   3795 |     return { success: false, error: true };
```
...
```ts
   3802 | ) => {
   3803 |   try {
   3804 |     await requireFinancePermission("finance.invoices.manage");
   3805 | 
>  3806 |     await prisma.feeMaster.create({
   3807 |       data: {
   3808 |         studentId: data.studentId,
   3809 |         term: data.term,
   3810 |         academicYear: data.academicYear,
```
...
```ts
   3828 | 
   3829 |   try {
   3830 |     await requireFinancePermission("finance.invoices.manage");
   3831 | 
>  3832 |     await prisma.feeMaster.update({
   3833 |       where: { id: data.id },
   3834 |       data: {
   3835 |         studentId: data.studentId,
   3836 |         term: data.term,
```
...
```ts
   3853 | 
   3854 |   try {
   3855 |     await requireFinancePermission("finance.invoices.manage");
   3856 | 
>  3857 |     await prisma.feeMaster.delete({ where: { id: parseInt(id) } });
   3858 |     return { success: true, error: false };
   3859 |   } catch (err) {
   3860 |     console.log("DELETE FEE MASTER ERROR:", err);
   3861 |     return { success: false, error: true };
```
...
```ts
   3865 | export const createFee = async (currentState: any, data: FeeSchema) => {
   3866 |   try {
   3867 |     await requireFinancePermission("finance.invoices.manage");
   3868 | 
>  3869 |     await prisma.fee.create({
   3870 |       data: {
   3871 |         masterId: data.masterId,
   3872 |         structureId: data.structureId,
   3873 |         amount: data.amount,
```
...
```ts
   3886 | 
   3887 |   try {
   3888 |     await requireFinancePermission("finance.invoices.manage");
   3889 | 
>  3890 |     await prisma.fee.update({
   3891 |       where: { id: data.id },
   3892 |       data: {
   3893 |         masterId: data.masterId,
   3894 |         structureId: data.structureId,
```
...
```ts
   3909 | 
   3910 |   try {
   3911 |     await requireFinancePermission("finance.invoices.manage");
   3912 | 
>  3913 |     await prisma.fee.delete({ where: { id: parseInt(id) } });
   3914 |     return { success: true, error: false };
   3915 |   } catch (err) {
   3916 |     console.log("DELETE FEE ERROR:", err);
   3917 |     return { success: false, error: true };
```
...
```ts
   ...
   4050 |         }
   4051 | 
>  4052 |         const payment = await tx.feePayment.create({
   4053 |           data: {
   4054 |             masterId: data.masterId,
   4055 | 
   4056 |             amount: data.amount,
```
...
```ts
   ...
   4209 |         /* ------------------------------------------------------------------ */
   4210 | 
>  4211 |         await tx.feePayment.update({
   4212 |           where: {
   4213 |             id: data.id!,
   4214 |           },
   4215 | 
...
```ts
   ...
   4333 |     }
   4334 | 
   4335 |     await prisma.$transaction(async (tx) => {
>  4336 |       await tx.feePayment.delete({
   4337 |         where: {
   4338 |           id: payment.id,
   4339 |         },
   4340 |       });
...
```ts
   ...
   4450 |   /* ------------------------------------------------------------------------ */
   4451 | 
>  4452 |   return prisma.feeMaster.create({
   4453 |     data: {
   4454 |       studentId,
   4455 | 
   4456 |       term,
...
```ts
   4480 |   currentState: CurrentState,
   4481 |   data: AttendanceSchema,
   4482 | ) => {
   4483 |   try {
>  4484 |     await prisma.attendance.create({
   4485 |       data: {
   4486 |         date: data.date,
   4487 |         present: data.present,
   4488 |         day: data.day,
```
...
```ts
   4503 | ) => {
   4504 |   if (!data.id) return { success: false, error: true };
   4505 | 
   4506 |   try {
>  4507 |     await prisma.attendance.update({
   4508 |       where: { id: data.id },
   4509 |       data: {
   4510 |         date: data.date,
   4511 |         present: data.present,
```
...
```ts
   4724 |          * The school has one current active
   4725 |          * term across the application.
   4726 |          */
   4727 |         if (data.isActive) {
>  4728 |           await tx.schoolTerm.updateMany({
   4729 |             where: {
   4730 |               isActive: true,
   4731 | 
   4732 |               ...(data.id
...
```ts
   4789 |           const termDateRangeChanged =
   4790 |             existing.startDate.getTime() !== startDate.getTime() ||
   4791 |             existing.endDate.getTime() !== endDate.getTime();
   4792 | 
>  4793 |           const updated = await tx.schoolTerm.update({
   4794 |             where: {
   4795 |               id: data.id,
   4796 |             },
   4797 | 
...
```ts
   ...
   4889 |         /* ------------------------------------------------------------ */
   4890 | 
>  4891 |         const created = await tx.schoolTerm.create({
   4892 |           data: {
   4893 |             academicYearId: data.academicYearId,
   4894 | 
   4895 |             name: data.name,
```
...
```ts
   ...
   5031 |       }
   5032 | 
>  5033 |       return tx.schoolAcademicYear.create({
   5034 |         data: {
   5035 |           name,
   5036 | 
   5037 |           startDate: data.startDate,
```
...
```ts
   5132 |         throw new Error("The academic year could not be found.");
   5133 |       }
   5134 | 
   5135 |       if (data.isActive) {
>  5136 |         await tx.schoolAcademicYear.updateMany({
   5137 |           where: {
   5138 |             isActive: true,
   5139 | 
   5140 |             NOT: {
...
```ts
   ...
   5149 |       }
   5150 | 
>  5151 |       return tx.schoolAcademicYear.update({
   5152 |         where: {
   5153 |           id: data.id,
   5154 |         },
   5155 | 
...
`src/lib/actions.ts:3969`

```ts
   3965 |   const userId = accessActor.actor.id;
   3966 | 
   3967 |   const actorRole =
   3968 |     grantingAssignment?.role.key?.trim().toLowerCase() ??
>  3969 |     accessActor.actor.legacyRole?.trim().toLowerCase() ??
   3970 |     null;
   3971 | 
   3972 |   const actorName =
   3973 |     accessActor.actor.displayName?.trim() ||
```
...
`src/lib/actions.ts:4585`

```ts
   4581 |     const userId = accessActor.actor.id;
   4582 | 
   4583 |     const actorRole =
   4584 |       grantingAssignment?.role.key?.trim().toLowerCase() ??
>  4585 |       accessActor.actor.legacyRole?.trim().toLowerCase() ??
   4586 |       null;
   4587 | 
   4588 |     const actorName =
   4589 |       accessActor.actor.displayName?.trim() ||
```
