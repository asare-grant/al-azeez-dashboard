...
Generated: 2026-08-18T00:04:33.646Z

## Executive Summary

- Files scanned: **726**
- Total findings: **864**
- Migration risk score: **3974**
- Good RBAC signals: **97**
...
```ts
     ...
     69 |   data: SubjectSchema,
     70 | ) => {
     71 |   try {
>    72 |     await prisma.subject.create({
     73 |       data: {
     74 |         name: data.name,
     75 |         teachers: {
     76 |           connect: data.teachers.map((teacherId) => ({ id: teacherId })),
```
...
```ts
     ...
     91 |   data: SubjectSchema,
     92 | ) => {
     93 |   try {
>    94 |     await prisma.subject.update({
     95 |       where: {
     96 |         id: data.id,
     97 |       },
     98 |       data: {
...
```ts
    116 |   data: FormData,
    117 | ) => {
    118 |   const id = data.get("id") as string;
    119 |   try {
>   120 |     await prisma.subject.delete({
    121 |       where: {
    122 |         id: parseInt(id),
    123 |       },
    124 |     });
...
```ts
    135 |   currentState: CurrentState,
    136 |   data: ClassSchema,
    137 | ) => {
    138 |   try {
>   139 |     await prisma.class.create({
    140 |       data,
    141 |     });
    142 | 
    143 |     // revalidatePath("/list/class");
```
...
```ts
    155 |   if (!data.id) {
    156 |     return { success: false, error: true };
    157 |   }
    158 |   try {
>   159 |     await prisma.class.update({
    160 |       where: {
    161 |         id: data.id,
    162 |       },
    163 |       data,
...
```ts
    176 |   data: FormData,
    177 | ) => {
    178 |   const id = data.get("id") as string;
    179 |   try {
>   180 |     await prisma.class.delete({
    181 |       where: {
    182 |         id: parseInt(id),
    183 |       },
    184 |     });
...
```ts
    205 |       lastName: data.surname,
    206 |       publicMetadata: { role: "teacher" },
    207 |     });
    208 | 
>   209 |     await prisma.teacher.create({
    210 |       data: {
    211 |         id: user.id,
...
```ts
    251 |       firstName: data.name,
    252 |       lastName: data.surname,
    253 |     });
    254 | 
>   255 |     await prisma.teacher.update({
    256 |       where: {
    257 |         id: data.id,
    258 |       },
    259 |       data: {
...
```ts
    292 |     const client = await clerkClient();
    293 | 
    294 |     await client.users.deleteUser(id);
    295 | 
>   296 |     await prisma.teacher.delete({
    297 |       where: {
    298 |         id: id,
    299 |       },
    300 |     });
...
```ts
    331 |       lastName: data.surname,
    332 |       publicMetadata: { role: "student" },
    333 |     });
    334 | 
>   335 |     await prisma.student.create({
    336 |       data: {
    337 |         id: user.id,
...
```ts
    377 |       firstName: data.name,
    378 |       lastName: data.surname,
    379 |     });
    380 | 
>   381 |     await prisma.student.update({
    382 |       where: {
    383 |         id: data.id,
    384 |       },
    385 |       data: {
...
```ts
    418 |     const client = await clerkClient();
    419 | 
    420 |     await client.users.deleteUser(id);
    421 | 
>   422 |     await prisma.student.delete({
    423 |       where: {
    424 |         id: id,
    425 |       },
    426 |     });
...
```ts
    451 |       publicMetadata: { role: "parent" },
    452 |     });
    453 | 
    454 |     // Create parent in Prisma
>   455 |     await prisma.parent.create({
    456 |       data: {
    457 |         id: user.id, // Clerk user id as parent id
...
```ts
    493 |       lastName: data.surname,
    494 |     });
    495 | 
    496 |     // Update Prisma parent record
>   497 |     await prisma.parent.update({
    498 |       where: { id: data.id },
    499 |       data: {
...
```ts
    533 |   try {
    534 |     const client = await clerkClient();
    535 | 
    536 |     // 1️⃣ Delete associated students first (if any)
>   537 |     await prisma.student.deleteMany({
    538 |       where: { parentId: id },
    539 |     });
    540 | 
    541 |     // 2️⃣ Delete parent from Prisma
```
...
```ts
    ...
    540 | 
    541 |     // 2️⃣ Delete parent from Prisma
>   542 |     await prisma.parent.delete({
    543 |       where: { id },
    544 |     });
    545 | 
    546 |     // 3️⃣ Delete parent from Clerk
```
...
```ts
    556 |       console.warn(
    557 |         "⚠️ Parent not found in Clerk. Continuing with Prisma deletion.",
    558 |       );
    559 |       try {
>   560 |         await prisma.parent.delete({ where: { id } });
    561 |         return { success: true, error: false };
    562 |       } catch (prismaErr) {
    563 |         console.error("❌ Error deleting parent from Prisma:", prismaErr);
    564 |       }
```
...
```ts
    ...
    625 |     }
    626 | 
>   627 |     await prisma.exam.create({
    628 |       data: {
    629 |         title: data.title,
    630 | 
    631 |         startTime: data.startTime,
```
...
```ts
    707 |     }
    708 | 
    709 |     const academicYear = data.academicYear.trim();
    710 | 
>   711 |     await prisma.exam.update({
    712 |       where: {
    713 |         id: data.id,
    714 |       },
    715 | 
...
```ts
    759 |   const { userId, sessionClaims } = await auth();
    760 |   const role = (sessionClaims?.metadata as { role?: string })?.role;
    761 | 
    762 |   try {
>   763 |     await prisma.exam.delete({
    764 |       where: {
    765 |         id: parseInt(id),
    766 |         ...(role === "teacher" ? { lesson: { teacherId: userId! } } : {}),
    767 |       },
```
...
```ts
    ...
    834 |     /* ---------------------------------------------------------------------- */
    835 | 
>   836 |     await prisma.lesson.create({
    837 |       data: {
    838 |         name:
    839 |           data.name,
    840 | 
...
```ts
    998 |       new Date(
    999 |         `1970-01-01T${data.endTime}:00Z`,
   1000 |       );
   1001 | 
>  1002 |     await prisma.lesson.update({
   1003 |       where: {
   1004 |         id:
   1005 |           data.id,
   1006 |       },
...
```ts
   ...
   1147 |     }
   1148 | 
>  1149 |     await prisma.lesson.delete({
   1150 |       where: {
   1151 |         id:
   1152 |           lessonId,
   1153 |       },
...
```ts
   ...
   1355 |       /* -------------------------------------------------------------------- */
   1356 | 
>  1357 |       await prisma.assignment.create({
   1358 |         data: {
   1359 |           title:
   1360 |             data.title,
   1361 | 
...
```ts
   ...
   1606 |       /* -------------------------------------------------------------------- */
   1607 | 
>  1608 |       await prisma.assignment.update({
   1609 |         where: {
   1610 |           id:
   1611 |             data.id,
   1612 |         },
...
```ts
   ...
   1787 |       /* -------------------------------------------------------------------- */
   1788 | 
>  1789 |       await prisma.assignment.delete({
   1790 |         where: {
   1791 |           id:
   1792 |             assignmentId,
   1793 |         },
...
```ts
   2219 |    * Keep NotificationEvent as historical/audit data,
   2220 |    * but hide deliveries that describe an obsolete
   2221 |    * event schedule.
   2222 |    */
>  2223 |   await tx.notification.updateMany({
   2224 |     where: {
   2225 |       archivedAt: null,
   2226 | 
   2227 |       event: {
```
...
```ts
   2313 |     }
   2314 | 
   2315 |     const event = await prisma.$transaction(
   2316 |       async (tx) => {
>  2317 |         const created = await tx.event.create({
   2318 |           data: {
   2319 |             title: values.title,
   2320 | 
   2321 |             description: values.description,
```
...
```ts
   2532 |           existing.startTime.getTime() !== values.startTime.getTime() ||
   2533 |           existing.endTime.getTime() !== values.endTime.getTime() ||
   2534 |           existing.classId !== (values.classId ?? null);
   2535 | 
>  2536 |         const updated = await tx.event.update({
   2537 |           where: {
   2538 |             id: eventId,
   2539 |           },
   2540 | 
...
```ts
   2819 |   currentState: CurrentState,
   2820 |   data: AnnouncementSchema,
   2821 | ) => {
   2822 |   try {
>  2823 |     await prisma.announcement.create({
   2824 |       data: {
   2825 |         title: data.title,
   2826 |         description: data.description,
   2827 |         date: new Date(data.date), // NEW
```
...
```ts
   2845 |     return { success: false, error: true };
   2846 |   }
   2847 | 
   2848 |   try {
>  2849 |     await prisma.announcement.update({
   2850 |       where: { id: typeof data.id === "string" ? parseInt(data.id) : data.id },
   2851 |       data: {
   2852 |         title: data.title,
   2853 |         description: data.description,
```
...
```ts
   2871 |   if (!id) {
   2872 |     return { success: false, error: true };
   2873 |   }
   2874 |   try {
>  2875 |     await prisma.announcement.delete({
   2876 |       where: { id: parseInt(id) },
   2877 |     });
   2878 |     // revalidatePath("/list/announcements");
   2879 |     return { success: true, error: false };
```
...
```ts
   2892 |   currentState: any,
   2893 |   data: FeeCategorySchema,
   2894 | ) => {
   2895 |   try {
>  2896 |     await prisma.feeCategory.create({
   2897 |       data: { name: data.name },
   2898 |     });
   2899 | 
   2900 |     return { success: true, error: false };
```
...
```ts
   2910 | ) => {
   2911 |   if (!data.id) return { success: false, error: true };
   2912 | 
   2913 |   try {
>  2914 |     await prisma.feeCategory.update({
   2915 |       where: { id: data.id },
   2916 |       data: { name: data.name },
   2917 |     });
   2918 | 
```
...
```ts
   2927 |   const id = data.get("id") as string;
   2928 |   if (!id) return { success: false, error: true };
   2929 | 
   2930 |   try {
>  2931 |     await prisma.feeCategory.delete({ where: { id: parseInt(id) } });
   2932 |     return { success: true, error: false };
   2933 |   } catch (err) {
   2934 |     console.log("DELETE FEE CATEGORY ERROR:", err);
   2935 |     return { success: false, error: true };
```
...
```ts
   2937 | };
   2938 | 
   2939 | export const createFeeType = async (currentState: any, data: FeeTypeSchema) => {
   2940 |   try {
>  2941 |     await prisma.feeType.create({
   2942 |       data: {
   2943 |         name: data.name,
   2944 |         categoryId: data.categoryId,
   2945 |       },
```
...
```ts
   2955 | export const updateFeeType = async (currentState: any, data: FeeTypeSchema) => {
   2956 |   if (!data.id) return { success: false, error: true };
   2957 | 
   2958 |   try {
>  2959 |     await prisma.feeType.update({
   2960 |       where: { id: data.id },
   2961 |       data: {
   2962 |         name: data.name,
   2963 |         categoryId: data.categoryId,
```
...
```ts
   2975 |   const id = data.get("id") as string;
   2976 |   if (!id) return { success: false, error: true };
   2977 | 
   2978 |   try {
>  2979 |     await prisma.feeType.delete({ where: { id: parseInt(id) } });
   2980 |     return { success: true, error: false };
   2981 |   } catch (err) {
   2982 |     console.log("DELETE FEE TYPE ERROR:", err);
   2983 |     return { success: false, error: true };
```
...
```ts
   2988 |   currentState: any,
   2989 |   data: FeeStructureSchema,
   2990 | ) => {
   2991 |   try {
>  2992 |     await prisma.feeStructure.create({
   2993 |       data: {
   2994 |         amount: data.amount,
   2995 |         studentType: data.studentType,
   2996 |         boardingType: data.boardingType,
```
...
```ts
   3013 | ) => {
   3014 |   if (!data.id) return { success: false, error: true };
   3015 | 
   3016 |   try {
>  3017 |     await prisma.feeStructure.update({
   3018 |       where: { id: data.id },
   3019 |       data: {
   3020 |         amount: data.amount,
   3021 |         studentType: data.studentType,
```
...
```ts
   3037 |   const id = data.get("id") as string;
   3038 |   if (!id) return { success: false, error: true };
   3039 | 
   3040 |   try {
>  3041 |     await prisma.feeStructure.delete({ where: { id: parseInt(id) } });
   3042 |     return { success: true, error: false };
   3043 |   } catch (err) {
   3044 |     console.log("DELETE FEE STRUCTURE ERROR:", err);
   3045 |     return { success: false, error: true };
```
...
```ts
   3050 |   currentState: any,
   3051 |   data: FeeMasterSchema,
   3052 | ) => {
   3053 |   try {
>  3054 |     await prisma.feeMaster.create({
   3055 |       data: {
   3056 |         studentId: data.studentId,
   3057 |         term: data.term,
   3058 |         academicYear: data.academicYear,
```
...
```ts
   3074 | ) => {
   3075 |   if (!data.id) return { success: false, error: true };
   3076 | 
   3077 |   try {
>  3078 |     await prisma.feeMaster.update({
   3079 |       where: { id: data.id },
   3080 |       data: {
   3081 |         studentId: data.studentId,
   3082 |         term: data.term,
```
...
```ts
   3097 |   const id = data.get("id") as string;
   3098 |   if (!id) return { success: false, error: true };
   3099 | 
   3100 |   try {
>  3101 |     await prisma.feeMaster.delete({ where: { id: parseInt(id) } });
   3102 |     return { success: true, error: false };
   3103 |   } catch (err) {
   3104 |     console.log("DELETE FEE MASTER ERROR:", err);
   3105 |     return { success: false, error: true };
```
...
```ts
   3107 | };
   3108 | 
   3109 | export const createFee = async (currentState: any, data: FeeSchema) => {
   3110 |   try {
>  3111 |     await prisma.fee.create({
   3112 |       data: {
   3113 |         masterId: data.masterId,
   3114 |         structureId: data.structureId,
   3115 |         amount: data.amount,
```
...
```ts
   3126 | export const updateFee = async (currentState: any, data: FeeSchema) => {
   3127 |   if (!data.id) return { success: false, error: true };
   3128 | 
   3129 |   try {
>  3130 |     await prisma.fee.update({
   3131 |       where: { id: data.id },
   3132 |       data: {
   3133 |         masterId: data.masterId,
   3134 |         structureId: data.structureId,
```
...
```ts
   3147 |   const id = data.get("id") as string;
   3148 |   if (!id) return { success: false, error: true };
   3149 | 
   3150 |   try {
>  3151 |     await prisma.fee.delete({ where: { id: parseInt(id) } });
   3152 |     return { success: true, error: false };
   3153 |   } catch (err) {
   3154 |     console.log("DELETE FEE ERROR:", err);
   3155 |     return { success: false, error: true };
```
...
```ts
   ...
   3254 |         }
   3255 | 
>  3256 |         const payment = await tx.feePayment.create({
   3257 |           data: {
   3258 |             masterId: data.masterId,
   3259 | 
   3260 |             amount: data.amount,
```
...
```ts
   3366 | ) => {
   3367 |   if (!data.id) return { success: false, error: true };
   3368 | 
   3369 |   try {
>  3370 |     await prisma.feePayment.update({
   3371 |       where: { id: data.id },
   3372 |       data: {
   3373 |         masterId: data.masterId,
   3374 |         amount: data.amount,
```
...
```ts
   3388 |   const id = data.get("id") as string;
   3389 |   if (!id) return { success: false, error: true };
   3390 | 
   3391 |   try {
>  3392 |     await prisma.feePayment.delete({ where: { id: parseInt(id) } });
   3393 |     return { success: true, error: false };
   3394 |   } catch (err) {
   3395 |     console.log("DELETE FEE PAYMENT ERROR:", err);
   3396 |     return { success: false, error: true };
```
...
```ts
   3431 |   // 3️⃣ Calculate total
   3432 |   const totalAmount = feeStructures.reduce((sum, fs) => sum + fs.amount, 0);
   3433 | 
   3434 |   // 4️⃣ Create FeeMaster
>  3435 |   const feeMaster = await prisma.feeMaster.create({
   3436 |     data: {
   3437 |       studentId,
   3438 |       term,
   3439 |       academicYear,
...
```ts
   3456 |   currentState: CurrentState,
   3457 |   data: AttendanceSchema,
   3458 | ) => {
   3459 |   try {
>  3460 |     await prisma.attendance.create({
   3461 |       data: {
   3462 |         date: data.date,
   3463 |         present: data.present,
   3464 |         day: data.day,
```
...
```ts
   3479 | ) => {
   3480 |   if (!data.id) return { success: false, error: true };
   3481 | 
   3482 |   try {
>  3483 |     await prisma.attendance.update({
   3484 |       where: { id: data.id },
   3485 |       data: {
   3486 |         date: data.date,
   3487 |         present: data.present,
```
...
```ts
   3654 |          * The school has one current active
   3655 |          * term across the application.
   3656 |          */
   3657 |         if (data.isActive) {
>  3658 |           await tx.schoolTerm.updateMany({
   3659 |             where: {
   3660 |               isActive: true,
   3661 | 
   3662 |               ...(data.id
...
```ts
   3719 |           const termDateRangeChanged =
   3720 |             existing.startDate.getTime() !== startDate.getTime() ||
   3721 |             existing.endDate.getTime() !== endDate.getTime();
   3722 | 
>  3723 |           const updated = await tx.schoolTerm.update({
   3724 |             where: {
   3725 |               id: data.id,
   3726 |             },
   3727 | 
...
```ts
   ...
   3819 |         /* ------------------------------------------------------------ */
   3820 | 
>  3821 |         const created = await tx.schoolTerm.create({
   3822 |           data: {
   3823 |             academicYearId: data.academicYearId,
   3824 | 
   3825 |             name: data.name,
```
...
```ts
   4347 |        * Only one academic year should
   4348 |        * be active at a time.
   4349 |        */
   4350 |       if (data.isActive) {
>  4351 |         await tx.schoolAcademicYear.updateMany({
   4352 |           where: {
   4353 |             isActive: true,
   4354 |           },
   4355 | 
...
```ts
   ...
   4360 |       }
   4361 | 
>  4362 |       return tx.schoolAcademicYear.create({
   4363 |         data: {
   4364 |           name,
   4365 | 
   4366 |           startDate: data.startDate,
```
...
```ts
   4464 |         throw new Error("The academic year could not be found.");
   4465 |       }
   4466 | 
   4467 |       if (data.isActive) {
>  4468 |         await tx.schoolAcademicYear.updateMany({
   4469 |           where: {
   4470 |             isActive: true,
   4471 | 
   4472 |             NOT: {
...
```ts
   ...
   4481 |       }
   4482 | 
>  4483 |       return tx.schoolAcademicYear.update({
   4484 |         where: {
   4485 |           id: data.id,
   4486 |         },
   4487 | 
...
```ts
    575 | ) => {
    576 |   const { userId, sessionClaims } = await auth();
    577 | 
    578 |   const role = (
>   579 |     sessionClaims?.metadata as {
    580 |       role?: string;
    581 |     }
    582 |   )?.role;
    583 | 
...
```ts
    669 | ) => {
    670 |   const { userId, sessionClaims } = await auth();
    671 | 
    672 |   const role = (
>   673 |     sessionClaims?.metadata as {
    674 |       role?: string;
    675 |     }
    676 |   )?.role;
    677 | 
...
```ts
    756 | ) => {
    757 |   const id = data.get("id") as string;
    758 | 
    759 |   const { userId, sessionClaims } = await auth();
>   760 |   const role = (sessionClaims?.metadata as { role?: string })?.role;
    761 | 
...
```ts
   2254 | ) => {
   2255 |   const { userId, sessionClaims } = await auth();
   2256 | 
   2257 |   const role = (
>  2258 |     sessionClaims?.metadata as {
   2259 |       role?: string;
   2260 |     }
   2261 |   )?.role;
   2262 | 
...
```ts
   2424 | ) => {
   2425 |   const { userId, sessionClaims } = await auth();
   2426 | 
   2427 |   const role = (
>  2428 |     sessionClaims?.metadata as {
   2429 |       role?: string;
   2430 |     }
   2431 |   )?.role;
   2432 | 
...
```ts
   2682 | ) => {
   2683 |   const { userId, sessionClaims } = await auth();
   2684 | 
   2685 |   const role = (
>  2686 |     sessionClaims?.metadata as {
   2687 |       role?: string;
   2688 |     }
   2689 |   )?.role;
   2690 | 
...
```ts
   3163 | ) => {
   3164 |   const { userId, sessionClaims } = await auth();
   3165 | 
   3166 |   const role = (
>  3167 |     sessionClaims?.metadata as {
   3168 |       role?: string;
   3169 |     }
   3170 |   )?.role;
   3171 | 
...
```ts
   3514 | }) => {
   3515 |   const { userId, sessionClaims } = await auth();
   3516 | 
   3517 |   const role = (
>  3518 |     sessionClaims?.metadata as {
   3519 |       role?: string;
   3520 |     }
   3521 |   )?.role;
   3522 | 
...
There are **209** findings requiring human classification.
...
`src/lib/actions.ts:587`

```ts
    583 | 
    584 |   try {
    585 |     const academicYear = data.academicYear.trim();
    586 | 
>   587 |     if (role === "teacher") {
    588 |       const teacherLesson = await prisma.lesson.findFirst({
    589 |         where: {
    590 |           teacherId: userId!,
    591 | 
```
...
```ts
    ...
    686 | 
>   687 |     if (role === "teacher") {
    688 |       const teacherLesson = await prisma.lesson.findFirst({
    689 |         where: {
    690 |           teacherId: userId!,
    691 | 
```
...
>   766 |         ...(role === "teacher" ? { lesson: { teacherId: userId! } } : {}),
    ...
    768 |     });
    769 | 
    770 |     // revalidatePath("/list/subjects");
```
