...
Generated: 2026-08-18T00:42:32.832Z

## Executive Summary

- Files scanned: **727**
- Total findings: **853**
- Migration risk score: **3938**
- Good RBAC signals: **100**
...
| MUTATION_RISK | 296 |
| LEGACY_ROLE | 129 |
| LEGACY_ROLE_GUARD | 105 |
| CENTRALIZED | 100 |
| AUTH_ONLY_REVIEW | 80 |
| ROLE_SCOPE_REVIEW | 78 |
| CLAIMS | 59 |
...
**File:** `src/lib/access-control/provisioning-service.ts:284`
...
```ts
    280 |       const displayName = `${identity.firstName} ${identity.lastName}`.trim();
    281 | 
    282 |       /* USER ACCOUNT */
    283 | 
>   284 |       await tx.userAccount.create({
    285 |         data: {
    286 |           id: clerkUser.id,
    287 | 
    288 |           username: identity.username,
```
...
**File:** `src/lib/access-control/provisioning-service.ts:337`
...
```ts
    333 |       });
    334 | 
    335 |       /* ROLE ASSIGNMENTS */
    336 | 
>   337 |       await tx.userRoleAssignment.createMany({
    338 |         data: roles.map((role) => ({
    339 |           userId: clerkUser.id,
    340 | 
    341 |           roleId: role.id,
```
...
**File:** `src/lib/access-control/provisioning-service.ts:353`
...
```ts
    349 |       });
    350 | 
    351 |       /* USER CREATED AUDIT */
    352 | 
>   353 |       await tx.accessAuditLog.create({
    354 |         data: {
    355 |           action: "USER_CREATED",
    356 | 
    357 |           actorId: actor.userId,
```
...
**File:** `src/lib/access-control/provisioning-service.ts:378`
...
```ts
    374 | 
    375 |       /* ROLE ASSIGNMENT AUDITS */
    376 | 
    377 |       if (roles.length > 0) {
>   378 |         await tx.accessAuditLog.createMany({
    379 |           data: roles.map((role) => ({
    380 |             action: "ROLE_ASSIGNED" as const,
    381 | 
    382 |             actorId: actor.userId,
```
...
```ts
     74 |   currentState: CurrentState,
     75 |   data: SubjectSchema,
     76 | ) => {
     77 |   try {
>    78 |     await prisma.subject.create({
     79 |       data: {
     80 |         name: data.name,
     81 |         teachers: {
     82 |           connect: data.teachers.map((teacherId) => ({ id: teacherId })),
```
...
```ts
     96 |   currentState: CurrentState,
     97 |   data: SubjectSchema,
     98 | ) => {
     99 |   try {
>   100 |     await prisma.subject.update({
    101 |       where: {
    102 |         id: data.id,
    103 |       },
    104 |       data: {
...
```ts
    122 |   data: FormData,
    123 | ) => {
    124 |   const id = data.get("id") as string;
    125 |   try {
>   126 |     await prisma.subject.delete({
    127 |       where: {
    128 |         id: parseInt(id),
    129 |       },
    130 |     });
...
```ts
    141 |   currentState: CurrentState,
    142 |   data: ClassSchema,
    143 | ) => {
    144 |   try {
>   145 |     await prisma.class.create({
    146 |       data,
    147 |     });
    148 | 
    149 |     // revalidatePath("/list/class");
```
...
```ts
    161 |   if (!data.id) {
    162 |     return { success: false, error: true };
    163 |   }
    164 |   try {
>   165 |     await prisma.class.update({
    166 |       where: {
    167 |         id: data.id,
    168 |       },
    169 |       data,
...
```ts
    182 |   data: FormData,
    183 | ) => {
    184 |   const id = data.get("id") as string;
    185 |   try {
>   186 |     await prisma.class.delete({
    187 |       where: {
    188 |         id: parseInt(id),
    189 |       },
    190 |     });
...
```ts
    211 |       lastName: data.surname,
    212 |       publicMetadata: { role: "teacher" },
    213 |     });
    214 | 
>   215 |     await prisma.teacher.create({
    216 |       data: {
    217 |         id: user.id,
    218 |         username: data.username,
    219 |         name: data.name,
```
...
```ts
    257 |       firstName: data.name,
    258 |       lastName: data.surname,
    259 |     });
    260 | 
>   261 |     await prisma.teacher.update({
    262 |       where: {
    263 |         id: data.id,
    264 |       },
    265 |       data: {
...
```ts
    298 |     const client = await clerkClient();
    299 | 
    300 |     await client.users.deleteUser(id);
    301 | 
>   302 |     await prisma.teacher.delete({
    303 |       where: {
    304 |         id: id,
    305 |       },
    306 |     });
...
```ts
    337 |       lastName: data.surname,
    338 |       publicMetadata: { role: "student" },
    339 |     });
    340 | 
>   341 |     await prisma.student.create({
    342 |       data: {
    343 |         id: user.id,
    344 |         username: data.username,
    345 |         name: data.name,
```
...
```ts
    383 |       firstName: data.name,
    384 |       lastName: data.surname,
    385 |     });
    386 | 
>   387 |     await prisma.student.update({
    388 |       where: {
    389 |         id: data.id,
    390 |       },
    391 |       data: {
...
```ts
    424 |     const client = await clerkClient();
    425 | 
    426 |     await client.users.deleteUser(id);
    427 | 
>   428 |     await prisma.student.delete({
    429 |       where: {
    430 |         id: id,
    431 |       },
    432 |     });
...
```ts
    457 |       publicMetadata: { role: "parent" },
    458 |     });
    459 | 
    460 |     // Create parent in Prisma
>   461 |     await prisma.parent.create({
    462 |       data: {
    463 |         id: user.id, // Clerk user id as parent id
    464 |         username: data.username,
    465 |         name: data.name,
```
...
```ts
    499 |       lastName: data.surname,
    500 |     });
    501 | 
    502 |     // Update Prisma parent record
>   503 |     await prisma.parent.update({
    504 |       where: { id: data.id },
    505 |       data: {
...
```ts
    539 |   try {
    540 |     const client = await clerkClient();
    541 | 
    542 |     // 1️⃣ Delete associated students first (if any)
>   543 |     await prisma.student.deleteMany({
    544 |       where: { parentId: id },
    545 |     });
    546 | 
    547 |     // 2️⃣ Delete parent from Prisma
```
...
```ts
    ...
    546 | 
    547 |     // 2️⃣ Delete parent from Prisma
>   548 |     await prisma.parent.delete({
    549 |       where: { id },
    550 |     });
    551 | 
    552 |     // 3️⃣ Delete parent from Clerk
```
...
```ts
    562 |       console.warn(
    563 |         "⚠️ Parent not found in Clerk. Continuing with Prisma deletion.",
    564 |       );
    565 |       try {
>   566 |         await prisma.parent.delete({ where: { id } });
    567 |         return { success: true, error: false };
    568 |       } catch (prismaErr) {
    569 |         console.error("❌ Error deleting parent from Prisma:", prismaErr);
    570 |       }
```
...
```ts
   ...
   1216 |     /* ---------------------------------------------------------------------- */
   1217 | 
>  1218 |     await prisma.lesson.create({
   1219 |       data: {
   1220 |         name:
   1221 |           data.name,
   1222 | 
...
```ts
   1380 |       new Date(
   1381 |         `1970-01-01T${data.endTime}:00Z`,
   1382 |       );
   1383 | 
>  1384 |     await prisma.lesson.update({
   1385 |       where: {
   1386 |         id:
   1387 |           data.id,
   1388 |       },
...
```ts
   ...
   1529 |     }
   1530 | 
>  1531 |     await prisma.lesson.delete({
   1532 |       where: {
   1533 |         id:
   1534 |           lessonId,
   1535 |       },
...
```ts
   ...
   1737 |       /* -------------------------------------------------------------------- */
   1738 | 
>  1739 |       await prisma.assignment.create({
   1740 |         data: {
   1741 |           title:
   1742 |             data.title,
   1743 | 
...
```ts
   ...
   1988 |       /* -------------------------------------------------------------------- */
   1989 | 
>  1990 |       await prisma.assignment.update({
   1991 |         where: {
   1992 |           id:
   1993 |             data.id,
   1994 |         },
...
```ts
   ...
   2169 |       /* -------------------------------------------------------------------- */
   2170 | 
>  2171 |       await prisma.assignment.delete({
   2172 |         where: {
   2173 |           id:
   2174 |             assignmentId,
   2175 |         },
...
```ts
   2601 |    * Keep NotificationEvent as historical/audit data,
   2602 |    * but hide deliveries that describe an obsolete
   2603 |    * event schedule.
   2604 |    */
>  2605 |   await tx.notification.updateMany({
   2606 |     where: {
   2607 |       archivedAt: null,
   2608 | 
   2609 |       event: {
```
...
```ts
   2695 |     }
   2696 | 
   2697 |     const event = await prisma.$transaction(
   2698 |       async (tx) => {
>  2699 |         const created = await tx.event.create({
   2700 |           data: {
   2701 |             title: values.title,
   2702 | 
   2703 |             description: values.description,
```
...
```ts
   2914 |           existing.startTime.getTime() !== values.startTime.getTime() ||
   2915 |           existing.endTime.getTime() !== values.endTime.getTime() ||
   2916 |           existing.classId !== (values.classId ?? null);
   2917 | 
>  2918 |         const updated = await tx.event.update({
   2919 |           where: {
   2920 |             id: eventId,
   2921 |           },
   2922 | 
...
```ts
   3201 |   currentState: CurrentState,
   3202 |   data: AnnouncementSchema,
   3203 | ) => {
   3204 |   try {
>  3205 |     await prisma.announcement.create({
   3206 |       data: {
   3207 |         title: data.title,
   3208 |         description: data.description,
   3209 |         date: new Date(data.date), // NEW
```
...
```ts
   3227 |     return { success: false, error: true };
   3228 |   }
   3229 | 
   3230 |   try {
>  3231 |     await prisma.announcement.update({
   3232 |       where: { id: typeof data.id === "string" ? parseInt(data.id) : data.id },
   3233 |       data: {
   3234 |         title: data.title,
   3235 |         description: data.description,
```
...
```ts
   3253 |   if (!id) {
   3254 |     return { success: false, error: true };
   3255 |   }
   3256 |   try {
>  3257 |     await prisma.announcement.delete({
   3258 |       where: { id: parseInt(id) },
   3259 |     });
   3260 |     // revalidatePath("/list/announcements");
   3261 |     return { success: true, error: false };
```
...
```ts
   3274 |   currentState: any,
   3275 |   data: FeeCategorySchema,
   3276 | ) => {
   3277 |   try {
>  3278 |     await prisma.feeCategory.create({
   3279 |       data: { name: data.name },
   3280 |     });
   3281 | 
   3282 |     return { success: true, error: false };
```
...
```ts
   3292 | ) => {
   3293 |   if (!data.id) return { success: false, error: true };
   3294 | 
   3295 |   try {
>  3296 |     await prisma.feeCategory.update({
   3297 |       where: { id: data.id },
   3298 |       data: { name: data.name },
   3299 |     });
   3300 | 
```
...
```ts
   3309 |   const id = data.get("id") as string;
   3310 |   if (!id) return { success: false, error: true };
   3311 | 
   3312 |   try {
>  3313 |     await prisma.feeCategory.delete({ where: { id: parseInt(id) } });
   3314 |     return { success: true, error: false };
   3315 |   } catch (err) {
   3316 |     console.log("DELETE FEE CATEGORY ERROR:", err);
   3317 |     return { success: false, error: true };
```
...
```ts
   3319 | };
   3320 | 
   3321 | export const createFeeType = async (currentState: any, data: FeeTypeSchema) => {
   3322 |   try {
>  3323 |     await prisma.feeType.create({
   3324 |       data: {
   3325 |         name: data.name,
   3326 |         categoryId: data.categoryId,
   3327 |       },
```
...
```ts
   3337 | export const updateFeeType = async (currentState: any, data: FeeTypeSchema) => {
   3338 |   if (!data.id) return { success: false, error: true };
   3339 | 
   3340 |   try {
>  3341 |     await prisma.feeType.update({
   3342 |       where: { id: data.id },
   3343 |       data: {
   3344 |         name: data.name,
   3345 |         categoryId: data.categoryId,
```
...
```ts
   3357 |   const id = data.get("id") as string;
   3358 |   if (!id) return { success: false, error: true };
   3359 | 
   3360 |   try {
>  3361 |     await prisma.feeType.delete({ where: { id: parseInt(id) } });
   3362 |     return { success: true, error: false };
   3363 |   } catch (err) {
   3364 |     console.log("DELETE FEE TYPE ERROR:", err);
   3365 |     return { success: false, error: true };
```
...
```ts
   3370 |   currentState: any,
   3371 |   data: FeeStructureSchema,
   3372 | ) => {
   3373 |   try {
>  3374 |     await prisma.feeStructure.create({
   3375 |       data: {
   3376 |         amount: data.amount,
   3377 |         studentType: data.studentType,
   3378 |         boardingType: data.boardingType,
```
...
```ts
   3395 | ) => {
   3396 |   if (!data.id) return { success: false, error: true };
   3397 | 
   3398 |   try {
>  3399 |     await prisma.feeStructure.update({
   3400 |       where: { id: data.id },
   3401 |       data: {
   3402 |         amount: data.amount,
   3403 |         studentType: data.studentType,
```
...
```ts
   3419 |   const id = data.get("id") as string;
   3420 |   if (!id) return { success: false, error: true };
   3421 | 
   3422 |   try {
>  3423 |     await prisma.feeStructure.delete({ where: { id: parseInt(id) } });
   3424 |     return { success: true, error: false };
   3425 |   } catch (err) {
   3426 |     console.log("DELETE FEE STRUCTURE ERROR:", err);
   3427 |     return { success: false, error: true };
```
...
```ts
   3432 |   currentState: any,
   3433 |   data: FeeMasterSchema,
   3434 | ) => {
   3435 |   try {
>  3436 |     await prisma.feeMaster.create({
   3437 |       data: {
   3438 |         studentId: data.studentId,
   3439 |         term: data.term,
   3440 |         academicYear: data.academicYear,
```
...
```ts
   3456 | ) => {
   3457 |   if (!data.id) return { success: false, error: true };
   3458 | 
   3459 |   try {
>  3460 |     await prisma.feeMaster.update({
   3461 |       where: { id: data.id },
   3462 |       data: {
   3463 |         studentId: data.studentId,
   3464 |         term: data.term,
```
...
```ts
   3479 |   const id = data.get("id") as string;
   3480 |   if (!id) return { success: false, error: true };
   3481 | 
   3482 |   try {
>  3483 |     await prisma.feeMaster.delete({ where: { id: parseInt(id) } });
   3484 |     return { success: true, error: false };
   3485 |   } catch (err) {
   3486 |     console.log("DELETE FEE MASTER ERROR:", err);
   3487 |     return { success: false, error: true };
```
...
```ts
   3489 | };
   3490 | 
   3491 | export const createFee = async (currentState: any, data: FeeSchema) => {
   3492 |   try {
>  3493 |     await prisma.fee.create({
   3494 |       data: {
   3495 |         masterId: data.masterId,
   3496 |         structureId: data.structureId,
   3497 |         amount: data.amount,
```
...
```ts
   3508 | export const updateFee = async (currentState: any, data: FeeSchema) => {
   3509 |   if (!data.id) return { success: false, error: true };
   3510 | 
   3511 |   try {
>  3512 |     await prisma.fee.update({
   3513 |       where: { id: data.id },
   3514 |       data: {
   3515 |         masterId: data.masterId,
   3516 |         structureId: data.structureId,
```
...
```ts
   3529 |   const id = data.get("id") as string;
   3530 |   if (!id) return { success: false, error: true };
   3531 | 
   3532 |   try {
>  3533 |     await prisma.fee.delete({ where: { id: parseInt(id) } });
   3534 |     return { success: true, error: false };
   3535 |   } catch (err) {
   3536 |     console.log("DELETE FEE ERROR:", err);
   3537 |     return { success: false, error: true };
```
...
```ts
   ...
   3636 |         }
   3637 | 
>  3638 |         const payment = await tx.feePayment.create({
   3639 |           data: {
   3640 |             masterId: data.masterId,
   3641 | 
   3642 |             amount: data.amount,
```
...
```ts
   3748 | ) => {
   3749 |   if (!data.id) return { success: false, error: true };
   3750 | 
   3751 |   try {
>  3752 |     await prisma.feePayment.update({
   3753 |       where: { id: data.id },
   3754 |       data: {
   3755 |         masterId: data.masterId,
   3756 |         amount: data.amount,
```
...
```ts
   3770 |   const id = data.get("id") as string;
   3771 |   if (!id) return { success: false, error: true };
   3772 | 
   3773 |   try {
>  3774 |     await prisma.feePayment.delete({ where: { id: parseInt(id) } });
   3775 |     return { success: true, error: false };
   3776 |   } catch (err) {
   3777 |     console.log("DELETE FEE PAYMENT ERROR:", err);
   3778 |     return { success: false, error: true };
```
...
```ts
   3813 |   // 3️⃣ Calculate total
   3814 |   const totalAmount = feeStructures.reduce((sum, fs) => sum + fs.amount, 0);
   3815 | 
   3816 |   // 4️⃣ Create FeeMaster
>  3817 |   const feeMaster = await prisma.feeMaster.create({
   3818 |     data: {
   3819 |       studentId,
   3820 |       term,
   3821 |       academicYear,
...
```ts
   3838 |   currentState: CurrentState,
   3839 |   data: AttendanceSchema,
   3840 | ) => {
   3841 |   try {
>  3842 |     await prisma.attendance.create({
   3843 |       data: {
   3844 |         date: data.date,
   3845 |         present: data.present,
   3846 |         day: data.day,
```
...
```ts
   3861 | ) => {
   3862 |   if (!data.id) return { success: false, error: true };
   3863 | 
   3864 |   try {
>  3865 |     await prisma.attendance.update({
   3866 |       where: { id: data.id },
   3867 |       data: {
   3868 |         date: data.date,
   3869 |         present: data.present,
```
...
```ts
   4036 |          * The school has one current active
   4037 |          * term across the application.
   4038 |          */
   4039 |         if (data.isActive) {
>  4040 |           await tx.schoolTerm.updateMany({
   4041 |             where: {
   4042 |               isActive: true,
   4043 | 
   4044 |               ...(data.id
...
```ts
   4101 |           const termDateRangeChanged =
   4102 |             existing.startDate.getTime() !== startDate.getTime() ||
   4103 |             existing.endDate.getTime() !== endDate.getTime();
   4104 | 
>  4105 |           const updated = await tx.schoolTerm.update({
   4106 |             where: {
   4107 |               id: data.id,
   4108 |             },
   4109 | 
...
```ts
   ...
   4201 |         /* ------------------------------------------------------------ */
   4202 | 
>  4203 |         const created = await tx.schoolTerm.create({
   4204 |           data: {
   4205 |             academicYearId: data.academicYearId,
   4206 | 
   4207 |             name: data.name,
```
...
```ts
   4729 |        * Only one academic year should
   4730 |        * be active at a time.
   4731 |        */
   4732 |       if (data.isActive) {
>  4733 |         await tx.schoolAcademicYear.updateMany({
   4734 |           where: {
   4735 |             isActive: true,
   4736 |           },
   4737 | 
...
```ts
   ...
   4742 |       }
   4743 | 
>  4744 |       return tx.schoolAcademicYear.create({
   4745 |         data: {
   4746 |           name,
   4747 | 
   4748 |           startDate: data.startDate,
```
...
```ts
   4846 |         throw new Error("The academic year could not be found.");
   4847 |       }
   4848 | 
   4849 |       if (data.isActive) {
>  4850 |         await tx.schoolAcademicYear.updateMany({
   4851 |           where: {
   4852 |             isActive: true,
   4853 | 
   4854 |             NOT: {
...
```ts
   ...
   4863 |       }
   4864 | 
>  4865 |       return tx.schoolAcademicYear.update({
   4866 |         where: {
   4867 |           id: data.id,
   4868 |         },
   4869 | 
...
```ts
   2636 | ) => {
   2637 |   const { userId, sessionClaims } = await auth();
   2638 | 
   2639 |   const role = (
>  2640 |     sessionClaims?.metadata as {
   2641 |       role?: string;
   2642 |     }
   2643 |   )?.role;
   2644 | 
...
```ts
   2806 | ) => {
   2807 |   const { userId, sessionClaims } = await auth();
   2808 | 
   2809 |   const role = (
>  2810 |     sessionClaims?.metadata as {
   2811 |       role?: string;
   2812 |     }
   2813 |   )?.role;
   2814 | 
...
```ts
   3064 | ) => {
   3065 |   const { userId, sessionClaims } = await auth();
   3066 | 
   3067 |   const role = (
>  3068 |     sessionClaims?.metadata as {
   3069 |       role?: string;
   3070 |     }
   3071 |   )?.role;
   3072 | 
...
```ts
   3545 | ) => {
   3546 |   const { userId, sessionClaims } = await auth();
   3547 | 
   3548 |   const role = (
>  3549 |     sessionClaims?.metadata as {
   3550 |       role?: string;
   3551 |     }
   3552 |   )?.role;
   3553 | 
...
```ts
   3896 | }) => {
   3897 |   const { userId, sessionClaims } = await auth();
   3898 | 
   3899 |   const role = (
>  3900 |     sessionClaims?.metadata as {
   3901 |       role?: string;
   3902 |     }
   3903 |   )?.role;
   3904 | 
...
There are **208** findings requiring human classification.
...
`src/lib/access-control/provisioning-service.ts:76`

```ts
     72 |   const actorAccount =
     73 |     accessActor.actor;
     74 | 
     75 |   const actorRole =
>    76 |     actorAccount.legacyRole
     77 |       ?.trim()
     78 |       .toLowerCase() ??
     79 |     accessActor.activeAssignments[0]
     80 |       ?.role.key
```
...
`src/lib/access-control/provisioning-service.ts:305`

```ts
    301 |           imageUrl: identity.imageUrl,
    302 | 
    303 |           status: "ACTIVE",
    304 | 
>   305 |           legacyRole: access.primaryRole,
    306 |         },
    307 |       });
    308 | 
    309 |       /* SCHOOL PROFILE */
