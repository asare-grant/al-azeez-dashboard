...
Generated: 2026-08-18T03:22:21.351Z

## Executive Summary

- Files scanned: **728**
- Total findings: **842**
- Migration risk score: **3856**
- Good RBAC signals: **107**
...
| LEGACY_ROLE | 136 |
| CENTRALIZED | 107 |
| LEGACY_ROLE_GUARD | 96 |
| ROLE_SCOPE_REVIEW | 79 |
| AUTH_ONLY_REVIEW | 72 |
| CLAIMS | 50 |
...
```ts
    ...
    653 |     /* ---------------------------------------------------------------------- */
    654 | 
>   655 |     await prisma.exam.create({
    656 |       data: {
    657 |         title: data.title,
    658 | 
    659 |         startTime: data.startTime,
```
...
```ts
   ...
   1004 |     /* ---------------------------------------------------------------------- */
   1005 | 
>  1006 |     await prisma.lesson.create({
   1007 |       data: {
   1008 |         name: data.name,
   1009 | 
   1010 |         day: data.day,
...
```ts
   1111 |     const startTime = new Date(`1970-01-01T${data.startTime}:00Z`);
   1112 | 
   1113 |     const endTime = new Date(`1970-01-01T${data.endTime}:00Z`);
   1114 | 
>  1115 |     await prisma.lesson.update({
   1116 |       where: {
   1117 |         id: data.id,
   1118 |       },
   1119 | 
...
```ts
   ...
   1206 |     }
   1207 | 
>  1208 |     await prisma.lesson.delete({
   1209 |       where: {
   1210 |         id: lessonId,
   1211 |       },
   1212 |     });
...
```ts
   ...
   1345 |     /* -------------------------------------------------------------------- */
   1346 | 
>  1347 |     await prisma.assignment.create({
   1348 |       data: {
   1349 |         title: data.title,
   1350 | 
   1351 |         startDate: data.startDate,
```
...
```ts
   ...
   1521 |     /* -------------------------------------------------------------------- */
   1522 | 
>  1523 |     await prisma.assignment.update({
   1524 |       where: {
   1525 |         id: data.id,
   1526 |       },
   1527 | 
...
```ts
   ...
   1645 |     /* -------------------------------------------------------------------- */
   1646 | 
>  1647 |     await prisma.assignment.delete({
   1648 |       where: {
   1649 |         id: assignmentId,
   1650 |       },
   1651 |     });
```
...
```ts
   2054 |    * Keep NotificationEvent as historical/audit data,
   2055 |    * but hide deliveries that describe an obsolete
   2056 |    * event schedule.
   2057 |    */
>  2058 |   await tx.notification.updateMany({
   2059 |     where: {
   2060 |       archivedAt: null,
   2061 | 
   2062 |       event: {
```
...
```ts
   2152 |     /* ---------------------------------------------------------------------- */
   2153 | 
   2154 |     const event = await prisma.$transaction(
   2155 |       async (tx) => {
>  2156 |         const created = await tx.event.create({
   2157 |           data: {
   2158 |             title: values.title,
   2159 | 
   2160 |             description: values.description,
```
...
```ts
   ...
   2389 |         /* -------------------------------------------------------------- */
   2390 | 
>  2391 |         const updated = await tx.event.update({
   2392 |           where: {
   2393 |             id: eventId,
   2394 |           },
   2395 | 
...
```ts
   2703 |   currentState: CurrentState,
   2704 |   data: AnnouncementSchema,
   2705 | ) => {
   2706 |   try {
>  2707 |     await prisma.announcement.create({
   2708 |       data: {
   2709 |         title: data.title,
   2710 |         description: data.description,
   2711 |         date: new Date(data.date), // NEW
```
...
```ts
   2729 |     return { success: false, error: true };
   2730 |   }
   2731 | 
   2732 |   try {
>  2733 |     await prisma.announcement.update({
   2734 |       where: { id: typeof data.id === "string" ? parseInt(data.id) : data.id },
   2735 |       data: {
   2736 |         title: data.title,
   2737 |         description: data.description,
```
...
```ts
   2755 |   if (!id) {
   2756 |     return { success: false, error: true };
   2757 |   }
   2758 |   try {
>  2759 |     await prisma.announcement.delete({
   2760 |       where: { id: parseInt(id) },
   2761 |     });
   2762 |     // revalidatePath("/list/announcements");
   2763 |     return { success: true, error: false };
```
...
```ts
   2776 |   currentState: any,
   2777 |   data: FeeCategorySchema,
   2778 | ) => {
   2779 |   try {
>  2780 |     await prisma.feeCategory.create({
   2781 |       data: { name: data.name },
   2782 |     });
   2783 | 
   2784 |     return { success: true, error: false };
```
...
```ts
   2794 | ) => {
   2795 |   if (!data.id) return { success: false, error: true };
   2796 | 
   2797 |   try {
>  2798 |     await prisma.feeCategory.update({
   2799 |       where: { id: data.id },
   2800 |       data: { name: data.name },
   2801 |     });
   2802 | 
```
...
```ts
   2811 |   const id = data.get("id") as string;
   2812 |   if (!id) return { success: false, error: true };
   2813 | 
   2814 |   try {
>  2815 |     await prisma.feeCategory.delete({ where: { id: parseInt(id) } });
   2816 |     return { success: true, error: false };
   2817 |   } catch (err) {
   2818 |     console.log("DELETE FEE CATEGORY ERROR:", err);
   2819 |     return { success: false, error: true };
```
...
```ts
   2821 | };
   2822 | 
   2823 | export const createFeeType = async (currentState: any, data: FeeTypeSchema) => {
   2824 |   try {
>  2825 |     await prisma.feeType.create({
   2826 |       data: {
   2827 |         name: data.name,
   2828 |         categoryId: data.categoryId,
   2829 |       },
```
...
```ts
   2839 | export const updateFeeType = async (currentState: any, data: FeeTypeSchema) => {
   2840 |   if (!data.id) return { success: false, error: true };
   2841 | 
   2842 |   try {
>  2843 |     await prisma.feeType.update({
   2844 |       where: { id: data.id },
   2845 |       data: {
   2846 |         name: data.name,
   2847 |         categoryId: data.categoryId,
```
...
```ts
   2859 |   const id = data.get("id") as string;
   2860 |   if (!id) return { success: false, error: true };
   2861 | 
   2862 |   try {
>  2863 |     await prisma.feeType.delete({ where: { id: parseInt(id) } });
   2864 |     return { success: true, error: false };
   2865 |   } catch (err) {
   2866 |     console.log("DELETE FEE TYPE ERROR:", err);
   2867 |     return { success: false, error: true };
```
...
```ts
   2872 |   currentState: any,
   2873 |   data: FeeStructureSchema,
   2874 | ) => {
   2875 |   try {
>  2876 |     await prisma.feeStructure.create({
   2877 |       data: {
   2878 |         amount: data.amount,
   2879 |         studentType: data.studentType,
   2880 |         boardingType: data.boardingType,
```
...
```ts
   2897 | ) => {
   2898 |   if (!data.id) return { success: false, error: true };
   2899 | 
   2900 |   try {
>  2901 |     await prisma.feeStructure.update({
   2902 |       where: { id: data.id },
   2903 |       data: {
   2904 |         amount: data.amount,
   2905 |         studentType: data.studentType,
```
...
```ts
   2921 |   const id = data.get("id") as string;
   2922 |   if (!id) return { success: false, error: true };
   2923 | 
   2924 |   try {
>  2925 |     await prisma.feeStructure.delete({ where: { id: parseInt(id) } });
   2926 |     return { success: true, error: false };
   2927 |   } catch (err) {
   2928 |     console.log("DELETE FEE STRUCTURE ERROR:", err);
   2929 |     return { success: false, error: true };
```
...
```ts
   2934 |   currentState: any,
   2935 |   data: FeeMasterSchema,
   2936 | ) => {
   2937 |   try {
>  2938 |     await prisma.feeMaster.create({
   2939 |       data: {
   2940 |         studentId: data.studentId,
   2941 |         term: data.term,
   2942 |         academicYear: data.academicYear,
```
...
```ts
   2958 | ) => {
   2959 |   if (!data.id) return { success: false, error: true };
   2960 | 
   2961 |   try {
>  2962 |     await prisma.feeMaster.update({
   2963 |       where: { id: data.id },
   2964 |       data: {
   2965 |         studentId: data.studentId,
   2966 |         term: data.term,
```
...
```ts
   2981 |   const id = data.get("id") as string;
   2982 |   if (!id) return { success: false, error: true };
   2983 | 
   2984 |   try {
>  2985 |     await prisma.feeMaster.delete({ where: { id: parseInt(id) } });
   2986 |     return { success: true, error: false };
   2987 |   } catch (err) {
   2988 |     console.log("DELETE FEE MASTER ERROR:", err);
   2989 |     return { success: false, error: true };
```
...
```ts
   2991 | };
   2992 | 
   2993 | export const createFee = async (currentState: any, data: FeeSchema) => {
   2994 |   try {
>  2995 |     await prisma.fee.create({
   2996 |       data: {
   2997 |         masterId: data.masterId,
   2998 |         structureId: data.structureId,
   2999 |         amount: data.amount,
```
...
```ts
   3010 | export const updateFee = async (currentState: any, data: FeeSchema) => {
   3011 |   if (!data.id) return { success: false, error: true };
   3012 | 
   3013 |   try {
>  3014 |     await prisma.fee.update({
   3015 |       where: { id: data.id },
   3016 |       data: {
   3017 |         masterId: data.masterId,
   3018 |         structureId: data.structureId,
```
...
```ts
   3031 |   const id = data.get("id") as string;
   3032 |   if (!id) return { success: false, error: true };
   3033 | 
   3034 |   try {
>  3035 |     await prisma.fee.delete({ where: { id: parseInt(id) } });
   3036 |     return { success: true, error: false };
   3037 |   } catch (err) {
   3038 |     console.log("DELETE FEE ERROR:", err);
   3039 |     return { success: false, error: true };
```
...
```ts
   ...
   3172 |         }
   3173 | 
>  3174 |         const payment = await tx.feePayment.create({
   3175 |           data: {
   3176 |             masterId: data.masterId,
   3177 | 
   3178 |             amount: data.amount,
```
...
```ts
   3284 | ) => {
   3285 |   if (!data.id) return { success: false, error: true };
   3286 | 
   3287 |   try {
>  3288 |     await prisma.feePayment.update({
   3289 |       where: { id: data.id },
   3290 |       data: {
   3291 |         masterId: data.masterId,
   3292 |         amount: data.amount,
```
...
```ts
   3306 |   const id = data.get("id") as string;
   3307 |   if (!id) return { success: false, error: true };
   3308 | 
   3309 |   try {
>  3310 |     await prisma.feePayment.delete({ where: { id: parseInt(id) } });
   3311 |     return { success: true, error: false };
   3312 |   } catch (err) {
   3313 |     console.log("DELETE FEE PAYMENT ERROR:", err);
   3314 |     return { success: false, error: true };
```
...
```ts
   3349 |   // 3️⃣ Calculate total
   3350 |   const totalAmount = feeStructures.reduce((sum, fs) => sum + fs.amount, 0);
   3351 | 
   3352 |   // 4️⃣ Create FeeMaster
>  3353 |   const feeMaster = await prisma.feeMaster.create({
   3354 |     data: {
   3355 |       studentId,
   3356 |       term,
   3357 |       academicYear,
...
```ts
   3374 |   currentState: CurrentState,
   3375 |   data: AttendanceSchema,
   3376 | ) => {
   3377 |   try {
>  3378 |     await prisma.attendance.create({
   3379 |       data: {
   3380 |         date: data.date,
   3381 |         present: data.present,
   3382 |         day: data.day,
```
...
```ts
   3397 | ) => {
   3398 |   if (!data.id) return { success: false, error: true };
   3399 | 
   3400 |   try {
>  3401 |     await prisma.attendance.update({
   3402 |       where: { id: data.id },
   3403 |       data: {
   3404 |         date: data.date,
   3405 |         present: data.present,
```
...
```ts
   3618 |          * The school has one current active
   3619 |          * term across the application.
   3620 |          */
   3621 |         if (data.isActive) {
>  3622 |           await tx.schoolTerm.updateMany({
   3623 |             where: {
   3624 |               isActive: true,
   3625 | 
   3626 |               ...(data.id
...
```ts
   3683 |           const termDateRangeChanged =
   3684 |             existing.startDate.getTime() !== startDate.getTime() ||
   3685 |             existing.endDate.getTime() !== endDate.getTime();
   3686 | 
>  3687 |           const updated = await tx.schoolTerm.update({
   3688 |             where: {
   3689 |               id: data.id,
   3690 |             },
   3691 | 
...
```ts
   ...
   3783 |         /* ------------------------------------------------------------ */
   3784 | 
>  3785 |         const created = await tx.schoolTerm.create({
   3786 |           data: {
   3787 |             academicYearId: data.academicYearId,
   3788 | 
   3789 |             name: data.name,
```
...
```ts
   4311 |        * Only one academic year should
   4312 |        * be active at a time.
   4313 |        */
   4314 |       if (data.isActive) {
>  4315 |         await tx.schoolAcademicYear.updateMany({
   4316 |           where: {
   4317 |             isActive: true,
   4318 |           },
   4319 | 
...
```ts
   ...
   4324 |       }
   4325 | 
>  4326 |       return tx.schoolAcademicYear.create({
   4327 |         data: {
   4328 |           name,
   4329 | 
   4330 |           startDate: data.startDate,
```
...
```ts
   4428 |         throw new Error("The academic year could not be found.");
   4429 |       }
   4430 | 
   4431 |       if (data.isActive) {
>  4432 |         await tx.schoolAcademicYear.updateMany({
   4433 |           where: {
   4434 |             isActive: true,
   4435 | 
   4436 |             NOT: {
...
```ts
   ...
   4445 |       }
   4446 | 
>  4447 |       return tx.schoolAcademicYear.update({
   4448 |         where: {
   4449 |           id: data.id,
   4450 |         },
   4451 | 
...
There are **216** findings requiring human classification.
...
`src/lib/actions.ts:3091`

```ts
   3087 |   const userId = accessActor.actor.id;
   3088 | 
   3089 |   const actorRole =
   3090 |     grantingAssignment?.role.key?.trim().toLowerCase() ??
>  3091 |     accessActor.actor.legacyRole?.trim().toLowerCase() ??
   3092 |     null;
   3093 | 
   3094 |   const actorName =
   3095 |     accessActor.actor.displayName?.trim() ||
```
...
`src/lib/actions.ts:3479`

```ts
   3475 |     const userId = accessActor.actor.id;
   3476 | 
   3477 |     const actorRole =
   3478 |       grantingAssignment?.role.key?.trim().toLowerCase() ??
>  3479 |       accessActor.actor.legacyRole?.trim().toLowerCase() ??
   3480 |       null;
   3481 | 
   3482 |     const actorName =
   3483 |       accessActor.actor.displayName?.trim() ||
```
