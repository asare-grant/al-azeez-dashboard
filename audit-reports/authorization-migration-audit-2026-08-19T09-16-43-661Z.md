...
Generated: 2026-08-19T09:16:43.683Z

## Executive Summary

- Files scanned: **734**
- Total findings: **761**
- Migration risk score: **3404**
- Good RBAC signals: **141**
...
| LEGACY_ROLE | 143 |
| CENTRALIZED | 131 |
| ROLE_SCOPE_REVIEW | 60 |
| LEGACY_ROLE_GUARD | 57 |
| AUTH_ONLY_REVIEW | 46 |
| CLAIMS | 23 |
...
```ts
   2739 |    * Keep NotificationEvent as historical/audit data,
   2740 |    * but hide deliveries that describe an obsolete
   2741 |    * event schedule.
   2742 |    */
>  2743 |   await tx.notification.updateMany({
   2744 |     where: {
   2745 |       archivedAt: null,
   2746 | 
   2747 |       event: {
```
...
```ts
   2837 |     /* ---------------------------------------------------------------------- */
   2838 | 
   2839 |     const event = await prisma.$transaction(
   2840 |       async (tx) => {
>  2841 |         const created = await tx.event.create({
   2842 |           data: {
   2843 |             title: values.title,
   2844 | 
   2845 |             description: values.description,
```
...
```ts
   ...
   3074 |         /* -------------------------------------------------------------- */
   3075 | 
>  3076 |         const updated = await tx.event.update({
   3077 |           where: {
   3078 |             id: eventId,
   3079 |           },
   3080 | 
...
```ts
   ...
   3427 |     /* ---------------------------------------------------------------------- */
   3428 | 
>  3429 |     const announcement = await prisma.announcement.create({
   3430 |       data: {
   3431 |         title: data.title,
   3432 | 
   3433 |         description: data.description,
```
...
```ts
   3549 |         message: "The announcement could not be found.",
   3550 |       };
   3551 |     }
   3552 | 
>  3553 |     await prisma.announcement.update({
   3554 |       where: {
   3555 |         id: announcementId,
   3556 |       },
   3557 | 
```
...
```ts
   3640 |         message: "The announcement could not be found.",
   3641 |       };
   3642 |     }
   3643 | 
>  3644 |     await prisma.announcement.delete({
   3645 |       where: {
   3646 |         id: announcementId,
   3647 |       },
   3648 |     });
```
...
```ts
   3694 | ) => {
   3695 |   try {
   3696 |     await requireFinancePermission("finance.structure.manage");
   3697 | 
>  3698 |     await prisma.feeCategory.create({
   3699 |       data: {
   3700 |         name: data.name,
   3701 |       },
   3702 |     });
...
```ts
   3725 | 
   3726 |   try {
   3727 |     await requireFinancePermission("finance.structure.manage");
   3728 | 
>  3729 |     await prisma.feeCategory.update({
   3730 |       where: { id: data.id },
   3731 |       data: { name: data.name },
   3732 |     });
   3733 | 
```
...
```ts
   3744 | 
   3745 |   try {
   3746 |     await requireFinancePermission("finance.structure.manage");
   3747 | 
>  3748 |     await prisma.feeCategory.delete({ where: { id: parseInt(id) } });
   3749 |     return { success: true, error: false };
   3750 |   } catch (err) {
   3751 |     console.log("DELETE FEE CATEGORY ERROR:", err);
   3752 |     return { success: false, error: true };
```
...
```ts
   3756 | export const createFeeType = async (currentState: any, data: FeeTypeSchema) => {
   3757 |   try {
   3758 |     await requireFinancePermission("finance.structure.manage");
   3759 | 
>  3760 |     await prisma.feeType.create({
   3761 |       data: {
   3762 |         name: data.name,
   3763 |         categoryId: data.categoryId,
   3764 |       },
```
...
```ts
   3776 | 
   3777 |   try {
   3778 |     await requireFinancePermission("finance.structure.manage");
   3779 | 
>  3780 |     await prisma.feeType.update({
   3781 |       where: { id: data.id },
   3782 |       data: {
   3783 |         name: data.name,
   3784 |         categoryId: data.categoryId,
```
...
```ts
   3798 | 
   3799 |   try {
   3800 |     await requireFinancePermission("finance.structure.manage");
   3801 | 
>  3802 |     await prisma.feeType.delete({ where: { id: parseInt(id) } });
   3803 |     return { success: true, error: false };
   3804 |   } catch (err) {
   3805 |     console.log("DELETE FEE TYPE ERROR:", err);
...
```ts
   3813 | ) => {
   3814 |   try {
   3815 |     await requireFinancePermission("finance.structure.manage");
   3816 | 
>  3817 |     await prisma.feeStructure.create({
   3818 |       data: {
   3819 |         amount: data.amount,
   3820 |         studentType: data.studentType,
   3821 |         boardingType: data.boardingType,
```
...
```ts
   3840 | 
   3841 |   try {
   3842 |     await requireFinancePermission("finance.structure.manage");
   3843 | 
>  3844 |     await prisma.feeStructure.update({
   3845 |       where: { id: data.id },
   3846 |       data: {
   3847 |         amount: data.amount,
   3848 |         studentType: data.studentType,
```
...
```ts
   3866 | 
   3867 |   try {
   3868 |     await requireFinancePermission("finance.structure.manage");
   3869 | 
>  3870 |     await prisma.feeStructure.delete({ where: { id: parseInt(id) } });
   3871 |     return { success: true, error: false };
   3872 |   } catch (err) {
   3873 |     console.log("DELETE FEE STRUCTURE ERROR:", err);
   3874 |     return { success: false, error: true };
```
...
```ts
   3881 | ) => {
   3882 |   try {
   3883 |     await requireFinancePermission("finance.invoices.manage");
   3884 | 
>  3885 |     await prisma.feeMaster.create({
   3886 |       data: {
   3887 |         studentId: data.studentId,
   3888 |         term: data.term,
   3889 |         academicYear: data.academicYear,
```
...
```ts
   3907 | 
   3908 |   try {
   3909 |     await requireFinancePermission("finance.invoices.manage");
   3910 | 
>  3911 |     await prisma.feeMaster.update({
   3912 |       where: { id: data.id },
   3913 |       data: {
   3914 |         studentId: data.studentId,
   3915 |         term: data.term,
```
...
```ts
   3932 | 
   3933 |   try {
   3934 |     await requireFinancePermission("finance.invoices.manage");
   3935 | 
>  3936 |     await prisma.feeMaster.delete({ where: { id: parseInt(id) } });
   3937 |     return { success: true, error: false };
   3938 |   } catch (err) {
   3939 |     console.log("DELETE FEE MASTER ERROR:", err);
...
```ts
   3944 | export const createFee = async (currentState: any, data: FeeSchema) => {
   3945 |   try {
   3946 |     await requireFinancePermission("finance.invoices.manage");
   3947 | 
>  3948 |     await prisma.fee.create({
   3949 |       data: {
   3950 |         masterId: data.masterId,
   3951 |         structureId: data.structureId,
   3952 |         amount: data.amount,
```
...
```ts
   3965 | 
   3966 |   try {
   3967 |     await requireFinancePermission("finance.invoices.manage");
   3968 | 
>  3969 |     await prisma.fee.update({
   3970 |       where: { id: data.id },
   3971 |       data: {
   3972 |         masterId: data.masterId,
   3973 |         structureId: data.structureId,
```
...
```ts
   3988 | 
   3989 |   try {
   3990 |     await requireFinancePermission("finance.invoices.manage");
   3991 | 
>  3992 |     await prisma.fee.delete({ where: { id: parseInt(id) } });
   3993 |     return { success: true, error: false };
   3994 |   } catch (err) {
   3995 |     console.log("DELETE FEE ERROR:", err);
   3996 |     return { success: false, error: true };
```
...
```ts
   ...
   4129 |         }
   4130 | 
>  4131 |         const payment = await tx.feePayment.create({
   4132 |           data: {
   4133 |             masterId: data.masterId,
   4134 | 
   4135 |             amount: data.amount,
```
...
```ts
   ...
   4288 |         /* ------------------------------------------------------------------ */
   4289 | 
>  4290 |         await tx.feePayment.update({
   4291 |           where: {
   4292 |             id: data.id!,
   4293 |           },
   4294 | 
...
```ts
   ...
   4412 |     }
   4413 | 
   4414 |     await prisma.$transaction(async (tx) => {
>  4415 |       await tx.feePayment.delete({
   4416 |         where: {
   4417 |           id: payment.id,
   4418 |         },
   4419 |       });
...
```ts
   ...
   4529 |   /* ------------------------------------------------------------------------ */
   4530 | 
>  4531 |   return prisma.feeMaster.create({
   4532 |     data: {
   4533 |       studentId,
   4534 | 
   4535 |       term,
...
```ts
   4559 |   currentState: CurrentState,
   4560 |   data: AttendanceSchema,
   4561 | ) => {
   4562 |   try {
>  4563 |     await prisma.attendance.create({
   4564 |       data: {
   4565 |         date: data.date,
   4566 |         present: data.present,
   4567 |         day: data.day,
```
...
```ts
   4582 | ) => {
   4583 |   if (!data.id) return { success: false, error: true };
   4584 | 
   4585 |   try {
>  4586 |     await prisma.attendance.update({
   4587 |       where: { id: data.id },
   4588 |       data: {
   4589 |         date: data.date,
   4590 |         present: data.present,
```
...
```ts
   4803 |          * The school has one current active
   4804 |          * term across the application.
   4805 |          */
   4806 |         if (data.isActive) {
>  4807 |           await tx.schoolTerm.updateMany({
   4808 |             where: {
   4809 |               isActive: true,
   4810 | 
   4811 |               ...(data.id
...
```ts
   4868 |           const termDateRangeChanged =
   4869 |             existing.startDate.getTime() !== startDate.getTime() ||
   4870 |             existing.endDate.getTime() !== endDate.getTime();
   4871 | 
>  4872 |           const updated = await tx.schoolTerm.update({
   4873 |             where: {
   4874 |               id: data.id,
   4875 |             },
   4876 | 
...
```ts
   ...
   4968 |         /* ------------------------------------------------------------ */
   4969 | 
>  4970 |         const created = await tx.schoolTerm.create({
   4971 |           data: {
   4972 |             academicYearId: data.academicYearId,
   4973 | 
   4974 |             name: data.name,
```
...
```ts
   ...
   5110 |       }
   5111 | 
>  5112 |       return tx.schoolAcademicYear.create({
   5113 |         data: {
   5114 |           name,
   5115 | 
   5116 |           startDate: data.startDate,
```
...
```ts
   5211 |         throw new Error("The academic year could not be found.");
   5212 |       }
   5213 | 
   5214 |       if (data.isActive) {
>  5215 |         await tx.schoolAcademicYear.updateMany({
   5216 |           where: {
   5217 |             isActive: true,
   5218 | 
   5219 |             NOT: {
...
```ts
   ...
   5228 |       }
   5229 | 
>  5230 |       return tx.schoolAcademicYear.update({
   5231 |         where: {
   5232 |           id: data.id,
   5233 |         },
   5234 | 
...
**File:** `src/app/(dashboard)/list/results/legacy/page.tsx:71`
...
```ts
     67 |   // ✅ Fix for Next.js 15 — unwrap searchParams
     68 |   const searchParams = await props.searchParams;
     69 | 
     70 |   const { userId, sessionClaims } = await auth();
>    71 |   const role = (sessionClaims?.metadata as { role?: string })?.role;
     72 |   const currentUserId = userId;
     73 | 
     74 |   // ✅ Define table columns
     75 |   const columns = [
```
...
**File:** `src/app/(dashboard)/list/results/legacy/page.tsx:90`
...
```ts
     86 |       className: "hidden md:table-cell",
     87 |     },
     88 |     { header: "Class", accessor: "class", className: "hidden md:table-cell" },
     89 |     { header: "Date", accessor: "date", className: "hidden md:table-cell" },
>    90 |     ...(role === "admin" || role === "teacher"
     91 |       ? [{ header: "Actions", accessor: "action" }]
     92 |       : []),
     93 |   ];
     94 | 
```
...
**File:** `src/app/(dashboard)/list/results/legacy/page.tsx:134`
...
```ts
    130 |       <td className="hidden md:table-cell">{item.className}</td>
    131 |       <td className="hidden md:table-cell">
    132 |         {new Intl.DateTimeFormat("en-US").format(item.startTime)}
    133 |       </td>
>   134 |       {(role === "admin" || role === "teacher") && (
    135 |         <td>
    136 |           <div className="flex items-center gap-2">
    137 |             <FormContainer table="result" type="update" data={item} />
    138 |             <FormContainer table="result" type="delete" id={item.id} />
```
...
**File:** `src/app/(dashboard)/list/results/legacy/page.tsx:391`
...
```ts
    387 |             </button>
    388 |             <button className="w-8 h-8 flex items-center justify-center rounded-full bg-[#FAE27C]">
    389 |               <Image src="/sort.png" alt="Sort" width={14} height={14} />
    390 |             </button>
>   391 |             {(role === "admin" || role === "teacher") && (
    392 |               <FormContainer table="result" type="create" />
    393 |             )}
    394 |           </div>
    395 |         </div>
```
...
There are **204** findings requiring human classification.
...
`src/app/(dashboard)/list/results/legacy/page.tsx:90`
...
`src/app/(dashboard)/list/results/legacy/page.tsx:134`
...
`src/app/(dashboard)/list/results/legacy/page.tsx:391`
...
`src/lib/actions.ts:4048`

```ts
   4044 |   const userId = accessActor.actor.id;
   4045 | 
   4046 |   const actorRole =
   4047 |     grantingAssignment?.role.key?.trim().toLowerCase() ??
>  4048 |     accessActor.actor.legacyRole?.trim().toLowerCase() ??
   4049 |     null;
   4050 | 
   4051 |   const actorName =
   4052 |     accessActor.actor.displayName?.trim() ||
```
...
`src/lib/actions.ts:4664`

```ts
   4660 |     const userId = accessActor.actor.id;
   4661 | 
   4662 |     const actorRole =
   4663 |       grantingAssignment?.role.key?.trim().toLowerCase() ??
>  4664 |       accessActor.actor.legacyRole?.trim().toLowerCase() ??
   4665 |       null;
   4666 | 
   4667 |     const actorName =
   4668 |       accessActor.actor.displayName?.trim() ||
```
