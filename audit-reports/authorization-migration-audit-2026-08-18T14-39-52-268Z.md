...
Generated: 2026-08-18T14:39:52.308Z

## Executive Summary

- Files scanned: **733**
- Total findings: **807**
- Migration risk score: **3657**
- Good RBAC signals: **120**
...
| LEGACY_ROLE | 143 |
| CENTRALIZED | 117 |
| LEGACY_ROLE_GUARD | 77 |
| ROLE_SCOPE_REVIEW | 73 |
| AUTH_ONLY_REVIEW | 59 |
| CLAIMS | 37 |
| LEGACY_ROUTE_GUARD | 4 |
| MUTATION_PROTECTED | 3 |
| REDIRECT | 1 |
| PREFIX_POLICY | 1 |
...
```ts
     ...
     81 |   data: SubjectSchema,
     82 | ) => {
     83 |   try {
>    84 |     await prisma.subject.create({
     85 |       data: {
     86 |         name: data.name,
     87 |         teachers: {
     88 |           connect: data.teachers.map((teacherId) => ({ id: teacherId })),
```
...
```ts
    102 |   currentState: CurrentState,
    103 |   data: SubjectSchema,
    104 | ) => {
    105 |   try {
>   106 |     await prisma.subject.update({
    107 |       where: {
    108 |         id: data.id,
    109 |       },
    110 |       data: {
...
```ts
    128 |   data: FormData,
    129 | ) => {
    130 |   const id = data.get("id") as string;
    131 |   try {
>   132 |     await prisma.subject.delete({
    133 |       where: {
    134 |         id: parseInt(id),
    135 |       },
    136 |     });
...
```ts
    ...
    336 |       publicMetadata: { role: "teacher" },
    337 |     });
    338 | 
>   339 |     await prisma.teacher.create({
    340 |       data: {
    341 |         id: user.id,
...
```ts
    ...
    383 |     });
    384 | 
>   385 |     await prisma.teacher.update({
    386 |       where: {
    387 |         id: data.id,
    388 |       },
    389 |       data: {
...
```ts
    ...
    425 | 
>   426 |     await prisma.teacher.delete({
    427 |       where: {
    428 |         id: id,
    429 |       },
    430 |     });
...
```ts
    461 |       lastName: data.surname,
    462 |       publicMetadata: { role: "student" },
    463 |     });
    464 | 
>   465 |     await prisma.student.create({
    466 |       data: {
    467 |         id: user.id,
...
```ts
    507 |       firstName: data.name,
    508 |       lastName: data.surname,
    509 |     });
    510 | 
>   511 |     await prisma.student.update({
    512 |       where: {
    513 |         id: data.id,
    514 |       },
    515 |       data: {
...
```ts
    548 |     const client = await clerkClient();
    549 | 
    550 |     await client.users.deleteUser(id);
    551 | 
>   552 |     await prisma.student.delete({
    553 |       where: {
    554 |         id: id,
    555 |       },
    556 |     });
...
```ts
    581 |       publicMetadata: { role: "parent" },
    582 |     });
    583 | 
    584 |     // Create parent in Prisma
>   585 |     await prisma.parent.create({
    586 |       data: {
    587 |         id: user.id, // Clerk user id as parent id
    588 |         username: data.username,
    589 |         name: data.name,
```
...
```ts
    623 |       lastName: data.surname,
    624 |     });
    625 | 
    626 |     // Update Prisma parent record
>   627 |     await prisma.parent.update({
    628 |       where: { id: data.id },
    629 |       data: {
    630 |         username: data.username,
    631 |         name: data.name,
```
...
```ts
    663 |   try {
    664 |     const client = await clerkClient();
    665 | 
    666 |     // 1️⃣ Delete associated students first (if any)
>   667 |     await prisma.student.deleteMany({
    668 |       where: { parentId: id },
    669 |     });
    670 | 
    671 |     // 2️⃣ Delete parent from Prisma
```
...
```ts
    ...
    670 | 
    671 |     // 2️⃣ Delete parent from Prisma
>   672 |     await prisma.parent.delete({
    673 |       where: { id },
    674 |     });
    675 | 
    676 |     // 3️⃣ Delete parent from Clerk
```
...
```ts
    686 |       console.warn(
    687 |         "⚠️ Parent not found in Clerk. Continuing with Prisma deletion.",
    688 |       );
    689 |       try {
>   690 |         await prisma.parent.delete({ where: { id } });
    691 |         return { success: true, error: false };
    692 |       } catch (prismaErr) {
    693 |         console.error("❌ Error deleting parent from Prisma:", prismaErr);
    694 |       }
```
...
```ts
    ...
    783 |     /* ---------------------------------------------------------------------- */
    784 | 
>   785 |     await prisma.exam.create({
    786 |       data: {
    787 |         title: data.title,
    788 | 
    789 |         startTime: data.startTime,
```
...
```ts
   ...
   1134 |     /* ---------------------------------------------------------------------- */
   1135 | 
>  1136 |     await prisma.lesson.create({
   1137 |       data: {
   1138 |         name: data.name,
   1139 | 
   1140 |         day: data.day,
...
```ts
   1241 |     const startTime = new Date(`1970-01-01T${data.startTime}:00Z`);
   1242 | 
   1243 |     const endTime = new Date(`1970-01-01T${data.endTime}:00Z`);
   1244 | 
>  1245 |     await prisma.lesson.update({
   1246 |       where: {
   1247 |         id: data.id,
   1248 |       },
   1249 | 
...
```ts
   ...
   1336 |     }
   1337 | 
>  1338 |     await prisma.lesson.delete({
   1339 |       where: {
   1340 |         id: lessonId,
   1341 |       },
   1342 |     });
...
```ts
   ...
   1475 |     /* -------------------------------------------------------------------- */
   1476 | 
>  1477 |     await prisma.assignment.create({
   1478 |       data: {
   1479 |         title: data.title,
   1480 | 
   1481 |         startDate: data.startDate,
```
...
```ts
   ...
   1651 |     /* -------------------------------------------------------------------- */
   1652 | 
>  1653 |     await prisma.assignment.update({
   1654 |       where: {
   1655 |         id: data.id,
   1656 |       },
   1657 | 
...
```ts
   ...
   1775 |     /* -------------------------------------------------------------------- */
   1776 | 
>  1777 |     await prisma.assignment.delete({
   1778 |       where: {
   1779 |         id: assignmentId,
   1780 |       },
   1781 |     });
```
...
```ts
   2184 |    * Keep NotificationEvent as historical/audit data,
   2185 |    * but hide deliveries that describe an obsolete
   2186 |    * event schedule.
   2187 |    */
>  2188 |   await tx.notification.updateMany({
   2189 |     where: {
   2190 |       archivedAt: null,
   2191 | 
   2192 |       event: {
```
...
```ts
   2282 |     /* ---------------------------------------------------------------------- */
   2283 | 
   2284 |     const event = await prisma.$transaction(
   2285 |       async (tx) => {
>  2286 |         const created = await tx.event.create({
   2287 |           data: {
   2288 |             title: values.title,
   2289 | 
   2290 |             description: values.description,
```
...
```ts
   ...
   2519 |         /* -------------------------------------------------------------- */
   2520 | 
>  2521 |         const updated = await tx.event.update({
   2522 |           where: {
   2523 |             id: eventId,
   2524 |           },
   2525 | 
...
```ts
   ...
   2884 |     /* ---------------------------------------------------------------------- */
   2885 | 
   2886 |     const announcement =
>  2887 |       await prisma.announcement.create({
   2888 |         data: {
   2889 |           title:
   2890 |             data.title,
   2891 | 
...
```ts
   3077 |           "The announcement could not be found.",
   3078 |       };
   3079 |     }
   3080 | 
>  3081 |     await prisma.announcement.update({
   3082 |       where: {
   3083 |         id:
   3084 |           announcementId,
   3085 |       },
...
```ts
   3222 |           "The announcement could not be found.",
   3223 |       };
   3224 |     }
   3225 | 
>  3226 |     await prisma.announcement.delete({
   3227 |       where: {
   3228 |         id:
   3229 |           announcementId,
   3230 |       },
...
```ts
   3303 |     await requireFinancePermission(
   3304 |       "finance.structure.manage",
   3305 |     );
   3306 | 
>  3307 |     await prisma.feeCategory.create({
   3308 |       data: {
   3309 |         name:
   3310 |           data.name,
   3311 |       },
...
```ts
   3346 |     await requireFinancePermission(
   3347 |       "finance.structure.manage",
   3348 |     );
   3349 | 
>  3350 |     await prisma.feeCategory.update({
   3351 |       where: { id: data.id },
   3352 |       data: { name: data.name },
   3353 |     });
   3354 | 
```
...
```ts
   3367 |     await requireFinancePermission(
   3368 |       "finance.structure.manage",
   3369 |     );
   3370 | 
>  3371 |     await prisma.feeCategory.delete({ where: { id: parseInt(id) } });
   3372 |     return { success: true, error: false };
   3373 |   } catch (err) {
   3374 |     console.log("DELETE FEE CATEGORY ERROR:", err);
   3375 |     return { success: false, error: true };
```
...
```ts
   3381 |     await requireFinancePermission(
   3382 |       "finance.structure.manage",
   3383 |     );
   3384 | 
>  3385 |     await prisma.feeType.create({
   3386 |       data: {
   3387 |         name: data.name,
   3388 |         categoryId: data.categoryId,
   3389 |       },
```
...
```ts
   3403 |     await requireFinancePermission(
   3404 |       "finance.structure.manage",
   3405 |     );
   3406 | 
>  3407 |     await prisma.feeType.update({
   3408 |       where: { id: data.id },
   3409 |       data: {
   3410 |         name: data.name,
   3411 |         categoryId: data.categoryId,
```
...
```ts
   3427 |     await requireFinancePermission(
   3428 |       "finance.structure.manage",
   3429 |     );
   3430 | 
>  3431 |     await prisma.feeType.delete({ where: { id: parseInt(id) } });
   3432 |     return { success: true, error: false };
   3433 |   } catch (err) {
   3434 |     console.log("DELETE FEE TYPE ERROR:", err);
   3435 |     return { success: false, error: true };
```
...
```ts
   3444 |     await requireFinancePermission(
   3445 |       "finance.structure.manage",
   3446 |     );
   3447 | 
>  3448 |     await prisma.feeStructure.create({
   3449 |       data: {
   3450 |         amount: data.amount,
   3451 |         studentType: data.studentType,
   3452 |         boardingType: data.boardingType,
```
...
```ts
   3473 |     await requireFinancePermission(
   3474 |       "finance.structure.manage",
   3475 |     );
   3476 | 
>  3477 |     await prisma.feeStructure.update({
   3478 |       where: { id: data.id },
   3479 |       data: {
   3480 |         amount: data.amount,
   3481 |         studentType: data.studentType,
```
...
```ts
   3501 |     await requireFinancePermission(
   3502 |       "finance.structure.manage",
   3503 |     );
   3504 | 
>  3505 |     await prisma.feeStructure.delete({ where: { id: parseInt(id) } });
   3506 |     return { success: true, error: false };
   3507 |   } catch (err) {
   3508 |     console.log("DELETE FEE STRUCTURE ERROR:", err);
   3509 |     return { success: false, error: true };
```
...
```ts
   3518 |     await requireFinancePermission(
   3519 |       "finance.invoices.manage",
   3520 |     );
   3521 | 
>  3522 |     await prisma.feeMaster.create({
   3523 |       data: {
   3524 |         studentId: data.studentId,
   3525 |         term: data.term,
   3526 |         academicYear: data.academicYear,
```
...
```ts
   3546 |     await requireFinancePermission(
   3547 |       "finance.invoices.manage",
   3548 |     );
   3549 | 
>  3550 |     await prisma.feeMaster.update({
   3551 |       where: { id: data.id },
   3552 |       data: {
   3553 |         studentId: data.studentId,
   3554 |         term: data.term,
```
...
```ts
   3573 |     await requireFinancePermission(
   3574 |       "finance.invoices.manage",
   3575 |     );
   3576 | 
>  3577 |     await prisma.feeMaster.delete({ where: { id: parseInt(id) } });
   3578 |     return { success: true, error: false };
   3579 |   } catch (err) {
   3580 |     console.log("DELETE FEE MASTER ERROR:", err);
   3581 |     return { success: false, error: true };
```
...
```ts
   3587 |     await requireFinancePermission(
   3588 |       "finance.invoices.manage",
   3589 |     );
   3590 | 
>  3591 |     await prisma.fee.create({
   3592 |       data: {
   3593 |         masterId: data.masterId,
   3594 |         structureId: data.structureId,
   3595 |         amount: data.amount,
```
...
```ts
   3610 |     await requireFinancePermission(
   3611 |       "finance.invoices.manage",
   3612 |     );
   3613 | 
>  3614 |     await prisma.fee.update({
   3615 |       where: { id: data.id },
   3616 |       data: {
   3617 |         masterId: data.masterId,
   3618 |         structureId: data.structureId,
```
...
```ts
   3635 |     await requireFinancePermission(
   3636 |       "finance.invoices.manage",
   3637 |     );
   3638 | 
>  3639 |     await prisma.fee.delete({ where: { id: parseInt(id) } });
   3640 |     return { success: true, error: false };
   3641 |   } catch (err) {
   3642 |     console.log("DELETE FEE ERROR:", err);
   3643 |     return { success: false, error: true };
```
...
```ts
   ...
   3776 |         }
   3777 | 
>  3778 |         const payment = await tx.feePayment.create({
   3779 |           data: {
   3780 |             masterId: data.masterId,
   3781 | 
   3782 |             amount: data.amount,
```
...
```ts
   ...
   3953 |         /* ------------------------------------------------------------------ */
   3954 | 
>  3955 |         await tx.feePayment.update({
   3956 |           where: {
   3957 |             id:
   3958 |               data.id!,
   3959 |           },
...
```ts
   4136 |     await prisma.$transaction(
   4137 |       async (
   4138 |         tx,
   4139 |       ) => {
>  4140 |         await tx.feePayment.delete({
   4141 |           where: {
   4142 |             id:
   4143 |               payment.id,
   4144 |           },
...
```ts
   ...
   4299 |   /* ------------------------------------------------------------------------ */
   4300 | 
>  4301 |   return prisma.feeMaster.create({
   4302 |     data: {
   4303 |       studentId,
   4304 | 
   4305 |       term,
...
```ts
   4338 |   currentState: CurrentState,
   4339 |   data: AttendanceSchema,
   4340 | ) => {
   4341 |   try {
>  4342 |     await prisma.attendance.create({
   4343 |       data: {
   4344 |         date: data.date,
   4345 |         present: data.present,
   4346 |         day: data.day,
```
...
```ts
   4361 | ) => {
   4362 |   if (!data.id) return { success: false, error: true };
   4363 | 
   4364 |   try {
>  4365 |     await prisma.attendance.update({
   4366 |       where: { id: data.id },
   4367 |       data: {
   4368 |         date: data.date,
   4369 |         present: data.present,
```
...
```ts
   4582 |          * The school has one current active
   4583 |          * term across the application.
   4584 |          */
   4585 |         if (data.isActive) {
>  4586 |           await tx.schoolTerm.updateMany({
   4587 |             where: {
   4588 |               isActive: true,
   4589 | 
   4590 |               ...(data.id
...
```ts
   4647 |           const termDateRangeChanged =
   4648 |             existing.startDate.getTime() !== startDate.getTime() ||
   4649 |             existing.endDate.getTime() !== endDate.getTime();
   4650 | 
>  4651 |           const updated = await tx.schoolTerm.update({
   4652 |             where: {
   4653 |               id: data.id,
   4654 |             },
   4655 | 
...
```ts
   ...
   4747 |         /* ------------------------------------------------------------ */
   4748 | 
>  4749 |         const created = await tx.schoolTerm.create({
   4750 |           data: {
   4751 |             academicYearId: data.academicYearId,
   4752 | 
   4753 |             name: data.name,
```
...
```ts
   4878 |        * Only one academic year should
   4879 |        * be active at a time.
   4880 |        */
   4881 |       if (data.isActive) {
>  4882 |         await tx.schoolAcademicYear.updateMany({
   4883 |           where: {
   4884 |             isActive: true,
   4885 |           },
   4886 | 
...
```ts
   ...
   4891 |       }
   4892 | 
>  4893 |       return tx.schoolAcademicYear.create({
   4894 |         data: {
   4895 |           name,
   4896 | 
   4897 |           startDate: data.startDate,
```
...
```ts
   4995 |         throw new Error("The academic year could not be found.");
   4996 |       }
   4997 | 
   4998 |       if (data.isActive) {
>  4999 |         await tx.schoolAcademicYear.updateMany({
   5000 |           where: {
   5001 |             isActive: true,
   5002 | 
   5003 |             NOT: {
...
```ts
   ...
   5012 |       }
   5013 | 
>  5014 |       return tx.schoolAcademicYear.update({
   5015 |         where: {
   5016 |           id: data.id,
   5017 |         },
   5018 | 
...
`src/lib/actions.ts:3695`

```ts
   3691 |   const userId = accessActor.actor.id;
   3692 | 
   3693 |   const actorRole =
   3694 |     grantingAssignment?.role.key?.trim().toLowerCase() ??
>  3695 |     accessActor.actor.legacyRole?.trim().toLowerCase() ??
   3696 |     null;
   3697 | 
   3698 |   const actorName =
   3699 |     accessActor.actor.displayName?.trim() ||
```
...
`src/lib/actions.ts:4443`

```ts
   4439 |     const userId = accessActor.actor.id;
   4440 | 
   4441 |     const actorRole =
   4442 |       grantingAssignment?.role.key?.trim().toLowerCase() ??
>  4443 |       accessActor.actor.legacyRole?.trim().toLowerCase() ??
   4444 |       null;
   4445 | 
   4446 |     const actorName =
   4447 |       accessActor.actor.displayName?.trim() ||
```
