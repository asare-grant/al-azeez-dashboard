...
Generated: 2026-08-18T15:52:44.672Z

## Executive Summary

- Files scanned: **733**
- Total findings: **797**
- Migration risk score: **3562**
- Good RBAC signals: **128**
...
| LEGACY_ROLE_GUARD | 69 |
| AUTH_ONLY_REVIEW | 56 |
| CLAIMS | 34 |
| MUTATION_PROTECTED | 7 |
...
```ts
    ...
    503 | 
    504 |     try {
>   505 |       await prisma.teacher.create({
    506 |         data: {
    507 |           id: user.id,
    508 | 
    509 |           username:
...
```ts
    ...
    677 |     /* ---------------------------------------------------------------------- */
    678 | 
>   679 |     await prisma.teacher.update({
    680 |       where: {
    681 |         id: data.id,
    682 |       },
    683 | 
...
```ts
    892 |       lastName: data.surname,
    893 |       publicMetadata: { role: "student" },
    894 |     });
    895 | 
>   896 |     await prisma.student.create({
    897 |       data: {
    898 |         id: user.id,
    899 |         username: data.username,
    900 |         name: data.name,
```
...
```ts
    938 |       firstName: data.name,
    939 |       lastName: data.surname,
    940 |     });
    941 | 
>   942 |     await prisma.student.update({
    943 |       where: {
    944 |         id: data.id,
    945 |       },
    946 |       data: {
...
```ts
    979 |     const client = await clerkClient();
    980 | 
    981 |     await client.users.deleteUser(id);
    982 | 
>   983 |     await prisma.student.delete({
    984 |       where: {
    985 |         id: id,
    986 |       },
    987 |     });
...
```ts
   1012 |       publicMetadata: { role: "parent" },
   1013 |     });
   1014 | 
   1015 |     // Create parent in Prisma
>  1016 |     await prisma.parent.create({
   1017 |       data: {
   1018 |         id: user.id, // Clerk user id as parent id
   1019 |         username: data.username,
   1020 |         name: data.name,
```
...
```ts
   1054 |       lastName: data.surname,
   1055 |     });
   1056 | 
   1057 |     // Update Prisma parent record
>  1058 |     await prisma.parent.update({
   1059 |       where: { id: data.id },
   1060 |       data: {
   1061 |         username: data.username,
   1062 |         name: data.name,
```
...
```ts
   1094 |   try {
   1095 |     const client = await clerkClient();
   1096 | 
   1097 |     // 1️⃣ Delete associated students first (if any)
>  1098 |     await prisma.student.deleteMany({
   1099 |       where: { parentId: id },
   1100 |     });
   1101 | 
   1102 |     // 2️⃣ Delete parent from Prisma
```
...
```ts
   ...
   1101 | 
   1102 |     // 2️⃣ Delete parent from Prisma
>  1103 |     await prisma.parent.delete({
   1104 |       where: { id },
   1105 |     });
   1106 | 
   1107 |     // 3️⃣ Delete parent from Clerk
```
...
```ts
   1117 |       console.warn(
   1118 |         "⚠️ Parent not found in Clerk. Continuing with Prisma deletion.",
   1119 |       );
   1120 |       try {
>  1121 |         await prisma.parent.delete({ where: { id } });
   1122 |         return { success: true, error: false };
   1123 |       } catch (prismaErr) {
   1124 |         console.error("❌ Error deleting parent from Prisma:", prismaErr);
   1125 |       }
```
...
```ts
   ...
   1214 |     /* ---------------------------------------------------------------------- */
   1215 | 
>  1216 |     await prisma.exam.create({
   1217 |       data: {
   1218 |         title: data.title,
   1219 | 
   1220 |         startTime: data.startTime,
```
...
```ts
   ...
   1565 |     /* ---------------------------------------------------------------------- */
   1566 | 
>  1567 |     await prisma.lesson.create({
   1568 |       data: {
   1569 |         name: data.name,
   1570 | 
   1571 |         day: data.day,
...
```ts
   1672 |     const startTime = new Date(`1970-01-01T${data.startTime}:00Z`);
   1673 | 
   1674 |     const endTime = new Date(`1970-01-01T${data.endTime}:00Z`);
   1675 | 
>  1676 |     await prisma.lesson.update({
   1677 |       where: {
   1678 |         id: data.id,
   1679 |       },
   1680 | 
...
```ts
   ...
   1767 |     }
   1768 | 
>  1769 |     await prisma.lesson.delete({
   1770 |       where: {
   1771 |         id: lessonId,
   1772 |       },
   1773 |     });
...
```ts
   ...
   1906 |     /* -------------------------------------------------------------------- */
   1907 | 
>  1908 |     await prisma.assignment.create({
   1909 |       data: {
   1910 |         title: data.title,
   1911 | 
   1912 |         startDate: data.startDate,
```
...
```ts
   ...
   2082 |     /* -------------------------------------------------------------------- */
   2083 | 
>  2084 |     await prisma.assignment.update({
   2085 |       where: {
   2086 |         id: data.id,
   2087 |       },
   2088 | 
...
```ts
   ...
   2206 |     /* -------------------------------------------------------------------- */
   2207 | 
>  2208 |     await prisma.assignment.delete({
   2209 |       where: {
   2210 |         id: assignmentId,
   2211 |       },
   2212 |     });
```
...
```ts
   2615 |    * Keep NotificationEvent as historical/audit data,
   2616 |    * but hide deliveries that describe an obsolete
   2617 |    * event schedule.
   2618 |    */
>  2619 |   await tx.notification.updateMany({
   2620 |     where: {
   2621 |       archivedAt: null,
   2622 | 
   2623 |       event: {
```
...
```ts
   2713 |     /* ---------------------------------------------------------------------- */
   2714 | 
   2715 |     const event = await prisma.$transaction(
   2716 |       async (tx) => {
>  2717 |         const created = await tx.event.create({
   2718 |           data: {
   2719 |             title: values.title,
   2720 | 
   2721 |             description: values.description,
```
...
```ts
   ...
   2950 |         /* -------------------------------------------------------------- */
   2951 | 
>  2952 |         const updated = await tx.event.update({
   2953 |           where: {
   2954 |             id: eventId,
   2955 |           },
   2956 | 
...
```ts
   ...
   3315 |     /* ---------------------------------------------------------------------- */
   3316 | 
   3317 |     const announcement =
>  3318 |       await prisma.announcement.create({
   3319 |         data: {
   3320 |           title:
   3321 |             data.title,
   3322 | 
...
```ts
   3508 |           "The announcement could not be found.",
   3509 |       };
   3510 |     }
   3511 | 
>  3512 |     await prisma.announcement.update({
   3513 |       where: {
   3514 |         id:
   3515 |           announcementId,
   3516 |       },
...
```ts
   3653 |           "The announcement could not be found.",
   3654 |       };
   3655 |     }
   3656 | 
>  3657 |     await prisma.announcement.delete({
   3658 |       where: {
   3659 |         id:
   3660 |           announcementId,
   3661 |       },
...
```ts
   3734 |     await requireFinancePermission(
   3735 |       "finance.structure.manage",
   3736 |     );
   3737 | 
>  3738 |     await prisma.feeCategory.create({
   3739 |       data: {
   3740 |         name:
   3741 |           data.name,
   3742 |       },
...
```ts
   3777 |     await requireFinancePermission(
   3778 |       "finance.structure.manage",
   3779 |     );
   3780 | 
>  3781 |     await prisma.feeCategory.update({
   3782 |       where: { id: data.id },
   3783 |       data: { name: data.name },
   3784 |     });
   3785 | 
```
...
```ts
   3798 |     await requireFinancePermission(
   3799 |       "finance.structure.manage",
   3800 |     );
   3801 | 
>  3802 |     await prisma.feeCategory.delete({ where: { id: parseInt(id) } });
   3803 |     return { success: true, error: false };
   3804 |   } catch (err) {
   3805 |     console.log("DELETE FEE CATEGORY ERROR:", err);
   3806 |     return { success: false, error: true };
```
...
```ts
   ...
   3813 |       "finance.structure.manage",
   3814 |     );
   3815 | 
>  3816 |     await prisma.feeType.create({
   3817 |       data: {
   3818 |         name: data.name,
   3819 |         categoryId: data.categoryId,
   3820 |       },
```
...
```ts
   3834 |     await requireFinancePermission(
   3835 |       "finance.structure.manage",
   3836 |     );
   3837 | 
>  3838 |     await prisma.feeType.update({
   3839 |       where: { id: data.id },
   3840 |       data: {
   3841 |         name: data.name,
   3842 |         categoryId: data.categoryId,
```
...
```ts
   3858 |     await requireFinancePermission(
   3859 |       "finance.structure.manage",
   3860 |     );
   3861 | 
>  3862 |     await prisma.feeType.delete({ where: { id: parseInt(id) } });
   3863 |     return { success: true, error: false };
   3864 |   } catch (err) {
   3865 |     console.log("DELETE FEE TYPE ERROR:", err);
   3866 |     return { success: false, error: true };
```
...
```ts
   3875 |     await requireFinancePermission(
   3876 |       "finance.structure.manage",
   3877 |     );
   3878 | 
>  3879 |     await prisma.feeStructure.create({
   3880 |       data: {
   3881 |         amount: data.amount,
   3882 |         studentType: data.studentType,
   3883 |         boardingType: data.boardingType,
```
...
```ts
   ...
   3905 |       "finance.structure.manage",
   3906 |     );
   3907 | 
>  3908 |     await prisma.feeStructure.update({
   3909 |       where: { id: data.id },
   3910 |       data: {
   3911 |         amount: data.amount,
   3912 |         studentType: data.studentType,
```
...
```ts
   3932 |     await requireFinancePermission(
   3933 |       "finance.structure.manage",
   3934 |     );
   3935 | 
>  3936 |     await prisma.feeStructure.delete({ where: { id: parseInt(id) } });
   3937 |     return { success: true, error: false };
   3938 |   } catch (err) {
   3939 |     console.log("DELETE FEE STRUCTURE ERROR:", err);
   3940 |     return { success: false, error: true };
```
...
```ts
   3949 |     await requireFinancePermission(
   3950 |       "finance.invoices.manage",
   3951 |     );
   3952 | 
>  3953 |     await prisma.feeMaster.create({
   3954 |       data: {
   3955 |         studentId: data.studentId,
   3956 |         term: data.term,
   3957 |         academicYear: data.academicYear,
```
...
```ts
   3977 |     await requireFinancePermission(
   3978 |       "finance.invoices.manage",
   3979 |     );
   3980 | 
>  3981 |     await prisma.feeMaster.update({
   3982 |       where: { id: data.id },
   3983 |       data: {
   3984 |         studentId: data.studentId,
   3985 |         term: data.term,
```
...
```ts
   4004 |     await requireFinancePermission(
   4005 |       "finance.invoices.manage",
   4006 |     );
   4007 | 
>  4008 |     await prisma.feeMaster.delete({ where: { id: parseInt(id) } });
   4009 |     return { success: true, error: false };
   4010 |   } catch (err) {
   4011 |     console.log("DELETE FEE MASTER ERROR:", err);
   4012 |     return { success: false, error: true };
```
...
```ts
   4018 |     await requireFinancePermission(
   4019 |       "finance.invoices.manage",
   4020 |     );
   4021 | 
>  4022 |     await prisma.fee.create({
   4023 |       data: {
   4024 |         masterId: data.masterId,
   4025 |         structureId: data.structureId,
   4026 |         amount: data.amount,
```
...
```ts
   4041 |     await requireFinancePermission(
   4042 |       "finance.invoices.manage",
   4043 |     );
   4044 | 
>  4045 |     await prisma.fee.update({
   4046 |       where: { id: data.id },
   4047 |       data: {
   4048 |         masterId: data.masterId,
   4049 |         structureId: data.structureId,
```
...
```ts
   4066 |     await requireFinancePermission(
   4067 |       "finance.invoices.manage",
   4068 |     );
   4069 | 
>  4070 |     await prisma.fee.delete({ where: { id: parseInt(id) } });
   4071 |     return { success: true, error: false };
   4072 |   } catch (err) {
   4073 |     console.log("DELETE FEE ERROR:", err);
   4074 |     return { success: false, error: true };
```
...
```ts
   ...
   4207 |         }
   4208 | 
>  4209 |         const payment = await tx.feePayment.create({
   4210 |           data: {
   4211 |             masterId: data.masterId,
   4212 | 
   4213 |             amount: data.amount,
```
...
```ts
   ...
   4384 |         /* ------------------------------------------------------------------ */
   4385 | 
>  4386 |         await tx.feePayment.update({
   4387 |           where: {
   4388 |             id:
   4389 |               data.id!,
   4390 |           },
...
```ts
   4567 |     await prisma.$transaction(
   4568 |       async (
   4569 |         tx,
   4570 |       ) => {
>  4571 |         await tx.feePayment.delete({
   4572 |           where: {
   4573 |             id:
   4574 |               payment.id,
   4575 |           },
...
```ts
   ...
   4730 |   /* ------------------------------------------------------------------------ */
   4731 | 
>  4732 |   return prisma.feeMaster.create({
   4733 |     data: {
   4734 |       studentId,
   4735 | 
   4736 |       term,
...
```ts
   4769 |   currentState: CurrentState,
   4770 |   data: AttendanceSchema,
   4771 | ) => {
   4772 |   try {
>  4773 |     await prisma.attendance.create({
   4774 |       data: {
   4775 |         date: data.date,
   4776 |         present: data.present,
   4777 |         day: data.day,
```
...
```ts
   4792 | ) => {
   4793 |   if (!data.id) return { success: false, error: true };
   4794 | 
   4795 |   try {
>  4796 |     await prisma.attendance.update({
   4797 |       where: { id: data.id },
   4798 |       data: {
   4799 |         date: data.date,
   4800 |         present: data.present,
```
...
```ts
   5013 |          * The school has one current active
   5014 |          * term across the application.
   5015 |          */
   5016 |         if (data.isActive) {
>  5017 |           await tx.schoolTerm.updateMany({
   5018 |             where: {
   5019 |               isActive: true,
   5020 | 
   5021 |               ...(data.id
...
```ts
   5078 |           const termDateRangeChanged =
   5079 |             existing.startDate.getTime() !== startDate.getTime() ||
   5080 |             existing.endDate.getTime() !== endDate.getTime();
   5081 | 
>  5082 |           const updated = await tx.schoolTerm.update({
   5083 |             where: {
   5084 |               id: data.id,
   5085 |             },
   5086 | 
...
```ts
   ...
   5178 |         /* ------------------------------------------------------------ */
   5179 | 
>  5180 |         const created = await tx.schoolTerm.create({
   5181 |           data: {
   5182 |             academicYearId: data.academicYearId,
   5183 | 
   5184 |             name: data.name,
```
...
```ts
   5309 |        * Only one academic year should
   5310 |        * be active at a time.
   5311 |        */
   5312 |       if (data.isActive) {
>  5313 |         await tx.schoolAcademicYear.updateMany({
   5314 |           where: {
   5315 |             isActive: true,
   5316 |           },
   5317 | 
...
```ts
   ...
   5322 |       }
   5323 | 
>  5324 |       return tx.schoolAcademicYear.create({
   5325 |         data: {
   5326 |           name,
   5327 | 
   5328 |           startDate: data.startDate,
```
...
```ts
   5426 |         throw new Error("The academic year could not be found.");
   5427 |       }
   5428 | 
   5429 |       if (data.isActive) {
>  5430 |         await tx.schoolAcademicYear.updateMany({
   5431 |           where: {
   5432 |             isActive: true,
   5433 | 
   5434 |             NOT: {
...
```ts
   ...
   5443 |       }
   5444 | 
>  5445 |       return tx.schoolAcademicYear.update({
   5446 |         where: {
   5447 |           id: data.id,
   5448 |         },
   5449 | 
...
`src/lib/actions.ts:4126`

```ts
   4122 |   const userId = accessActor.actor.id;
   4123 | 
   4124 |   const actorRole =
   4125 |     grantingAssignment?.role.key?.trim().toLowerCase() ??
>  4126 |     accessActor.actor.legacyRole?.trim().toLowerCase() ??
   4127 |     null;
   4128 | 
   4129 |   const actorName =
   4130 |     accessActor.actor.displayName?.trim() ||
```
...
`src/lib/actions.ts:4874`

```ts
   4870 |     const userId = accessActor.actor.id;
   4871 | 
   4872 |     const actorRole =
   4873 |       grantingAssignment?.role.key?.trim().toLowerCase() ??
>  4874 |       accessActor.actor.legacyRole?.trim().toLowerCase() ??
   4875 |       null;
   4876 | 
   4877 |     const actorName =
   4878 |       accessActor.actor.displayName?.trim() ||
```
