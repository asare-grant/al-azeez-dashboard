...
Generated: 2026-08-18T12:23:55.086Z

## Executive Summary

- Files scanned: **731**
- Total findings: **826**
- Migration risk score: **3789**
- Good RBAC signals: **112**
...
| LEGACY_ROLE | 138 |
| CENTRALIZED | 112 |
| LEGACY_ROLE_GUARD | 89 |
| ROLE_SCOPE_REVIEW | 75 |
| AUTH_ONLY_REVIEW | 66 |
| CLAIMS | 44 |
...
```ts
   ...
   3437 |         /* ------------------------------------------------------------------ */
   3438 | 
>  3439 |         await tx.feePayment.update({
   3440 |           where: {
   3441 |             id:
   3442 |               data.id!,
   3443 |           },
...
```ts
   3620 |     await prisma.$transaction(
   3621 |       async (
   3622 |         tx,
   3623 |       ) => {
>  3624 |         await tx.feePayment.delete({
   3625 |           where: {
   3626 |             id:
   3627 |               payment.id,
   3628 |           },
...
```ts
   ...
   3783 |   /* ------------------------------------------------------------------------ */
   3784 | 
>  3785 |   return prisma.feeMaster.create({
   3786 |     data: {
   3787 |       studentId,
   3788 | 
   3789 |       term,
...
```ts
   3822 |   currentState: CurrentState,
   3823 |   data: AttendanceSchema,
   3824 | ) => {
   3825 |   try {
>  3826 |     await prisma.attendance.create({
   3827 |       data: {
   3828 |         date: data.date,
   3829 |         present: data.present,
   3830 |         day: data.day,
```
...
```ts
   3845 | ) => {
   3846 |   if (!data.id) return { success: false, error: true };
   3847 | 
   3848 |   try {
>  3849 |     await prisma.attendance.update({
   3850 |       where: { id: data.id },
   3851 |       data: {
   3852 |         date: data.date,
   3853 |         present: data.present,
```
...
```ts
   4066 |          * The school has one current active
   4067 |          * term across the application.
   4068 |          */
   4069 |         if (data.isActive) {
>  4070 |           await tx.schoolTerm.updateMany({
   4071 |             where: {
   4072 |               isActive: true,
   4073 | 
   4074 |               ...(data.id
...
```ts
   4131 |           const termDateRangeChanged =
   4132 |             existing.startDate.getTime() !== startDate.getTime() ||
   4133 |             existing.endDate.getTime() !== endDate.getTime();
   4134 | 
>  4135 |           const updated = await tx.schoolTerm.update({
   4136 |             where: {
   4137 |               id: data.id,
   4138 |             },
   4139 | 
...
```ts
   ...
   4231 |         /* ------------------------------------------------------------ */
   4232 | 
>  4233 |         const created = await tx.schoolTerm.create({
   4234 |           data: {
   4235 |             academicYearId: data.academicYearId,
   4236 | 
   4237 |             name: data.name,
```
...
```ts
   4362 |        * Only one academic year should
   4363 |        * be active at a time.
   4364 |        */
   4365 |       if (data.isActive) {
>  4366 |         await tx.schoolAcademicYear.updateMany({
   4367 |           where: {
   4368 |             isActive: true,
   4369 |           },
   4370 | 
...
```ts
   ...
   4375 |       }
   4376 | 
>  4377 |       return tx.schoolAcademicYear.create({
   4378 |         data: {
   4379 |           name,
   4380 | 
   4381 |           startDate: data.startDate,
```
...
```ts
   4479 |         throw new Error("The academic year could not be found.");
   4480 |       }
   4481 | 
   4482 |       if (data.isActive) {
>  4483 |         await tx.schoolAcademicYear.updateMany({
   4484 |           where: {
   4485 |             isActive: true,
   4486 | 
   4487 |             NOT: {
...
```ts
   ...
   4496 |       }
   4497 | 
>  4498 |       return tx.schoolAcademicYear.update({
   4499 |         where: {
   4500 |           id: data.id,
   4501 |         },
   4502 | 
...
There are **214** findings requiring human classification.
...
`src/lib/actions.ts:3927`

```ts
   3923 |     const userId = accessActor.actor.id;
   3924 | 
   3925 |     const actorRole =
   3926 |       grantingAssignment?.role.key?.trim().toLowerCase() ??
>  3927 |       accessActor.actor.legacyRole?.trim().toLowerCase() ??
   3928 |       null;
   3929 | 
   3930 |     const actorName =
   3931 |       accessActor.actor.displayName?.trim() ||
```
