...
export default function BusFeeTable({
  ...
  classes: ClassLite[];
  busStudents: { studentId: string }[];
}) {
  const [selectedClass, setSelectedClass] = useState("");
  const [selectedMonth, setSelectedMonth] = useState(new Date());
  const [selectedStudent, setSelectedStudent] = useState("");
  const [payments, setPayments] = useState<BusFeePayment[]>([]);
  const [activeStudentIds, setActiveStudentIds] = useState(
    busStudents.map((s) => s.studentId),
  );
  ...
  const addStudentToBus = async () => {
    if (!selectedStudent) {
      toast.error("Please select a student");
      return;
    }

    await GlobalApi.AddBusFeeStudent(selectedStudent);
    ...
    setSelectedStudent("");
    toast.success("Student added to bus fee list");
  };
  ...
  const onCellValueChanged = async (e: CellValueChangedEvent<RowDataType>) => {
    ...
    try {
      const saved = await GlobalApi.UpsertBusFee({
      ...
      toast.error("Failed to save bus fee");
  ...
  }, [payments, busListStudents, selectedMonth]);
  ...
  const totalPerDay = useMemo(() => {
    ...
    getDays().forEach((day) => {
      totals[day] = payments
        .filter(
          (p) =>
            p.day === day && busListStudents.some((s) => s.id === p.studentId),
        )
    ...
    <div>
      <div className="my-6 flex flex-col gap-4 rounded-lg border p-5">
        ...
        <div className="flex flex-col gap-3 rounded-md bg-gray-50 p-4 sm:flex-row sm:items-center">
          ...
          <Button onClick={addStudentToBus}>Add Student</Button>
        </div>
      </div>
      ...
      <div className="mt-4 rounded-lg bg-gray-50 p-3">
        <h3 className="mb-2 font-semibold">Total Bus Fee per Day:</h3>
