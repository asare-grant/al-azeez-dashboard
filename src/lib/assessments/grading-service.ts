...
import { calculateTimeSpentSeconds, hasAttemptExpired } from "./timing";

import { AssessmentError } from "./errors";

import { withSerializableRetry } from "./transaction";

import { createAssessmentAudit } from "./audit";

import { syncAssessmentResult } from "@/lib/results/assessment-result-sync";

import type { AssessmentGradingSummary } from "./types";
...
function getGradingErrorResult(error: unknown): Extract<
  GradeAttemptResult,
  {
    success: false;
  }
    ...
    switch (error.message) {
      case "ATTEMPT_NOT_FOUND":
        return {
          ...
          message: "The assessment attempt could not be found.",
          retryable: false,
        };
      ...
      case "ATTEMPT_NOT_ACTIVE":
        return {
          ...
          message: "This assessment attempt is no longer active.",
          retryable: false,
        };
      ...
      case "UNANSWERED_QUESTIONS_NOT_ALLOWED":
        return {
          success: false,
          code: "UNANSWERED_QUESTIONS_NOT_ALLOWED",
          message: "Answer every question before submitting this assessment.",
          retryable: false,
        };
      ...
      case "COMPLETED_ATTEMPT_WITHOUT_RESULT":
        return {
          success: false,
          code: "COMPLETED_ATTEMPT_WITHOUT_RESULT",
          message: "The completed attempt could not be matched to a result.",
          retryable: false,
        };
    }
...
export async function gradeAttemptForStudent(
  ...
  try {
        ...
        async (tx) => {
          ...
          if (
            ...
            return {
              ...
              remarks:
                completedAttempt.result.remarks ?? "Assessment completed.",
              ...
              showCorrectAnswers:
                completedAttempt.assessment.showCorrectAnswers,
          ...
          if (
            ...
            existingAttempt.submissionToken !== submissionToken
          ) {
            throw new AssessmentError(
              "ATTEMPT_SUBMITTING",
          ...
          /*
           * Synchronise the authoritative assessment
           * Result row and invalidate any matching
           * generated DRAFT report card when the
           * academic result actually changed.
           */
    ...
    console.error("GRADE ASSESSMENT ATTEMPT ERROR:", {
      assessmentId,
      attemptId,
      studentId,
      submissionMode,
    ...
    return getGradingErrorResult(error);
  }
}
