...
import type { ClassTermReport, StudentTermReport } from "@/lib/academic-engine";

import { mapReportCardData, mapReportCardSubjectData } from "./mapper";

type PersistStudentReportResult = {
  reportCardId: number;

  operation: "GENERATED" | "REGENERATED" | "LOCKED";
};
...
async function persistStudentReport({
  ...
  const classId = report.period.class?.id ?? report.student.class.id;
  ...
  const existing = await tx.reportCard.findUnique({
    where: {
      studentId_academicYear_termId_classId: {
        ...
        academicYear: report.period.academicYear,

        termId: report.period.term.id,
  ...
  const reportData = mapReportCardData({
    report,
    generatedById,
  });
  ...
  const reportCard = existing
    ? await tx.reportCard.update({
        ...
        data: {
          ...
          /* -------------------------------------------------------------- */
          /*              FRESH ACADEMIC SNAPSHOT CREATED                  */
          /* -------------------------------------------------------------- */
          ...
          /*
           * Regeneration creates a new academic snapshot.
           *
           * Even if the previous snapshot had already entered
           * review, the new one must go through review again.
           */
          reviewStatus: "DRAFT",
  ...
  if (report.subjects.length > 0) {
    await tx.reportCardSubject.createMany({
      data: report.subjects.map((subject) =>
  ...
  return {
    ...
    operation: existing ? "REGENERATED" : "GENERATED",
  };
}
...
export async function persistClassTermReport({
  ...
  return prisma.$transaction(
    async (tx) => {
      const results: PersistStudentReportResult[] = [];
      ...
      for (const studentReport of report.students) {
        const persisted = await persistStudentReport({
          tx,
      ...
      return {
        ...
        generatedCount: results.filter(
          (result) => result.operation === "GENERATED",
        ).length,

        regeneratedCount: results.filter(
          (result) => result.operation === "REGENERATED",
        ).length,

        lockedCount: results.filter((result) => result.operation === "LOCKED")
          .length,

        reportCardIds: results.map((result) => result.reportCardId),
      };
    },
    {
