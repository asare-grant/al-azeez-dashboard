...
import {
  Prisma,
  type ReportCardCalculationStatus,
} from "@prisma/client";
...
function resolveStudentCalculationStatus(
  ...
  if (
    ...
    report.summary.completedSubjectCount <=
      0
  ) {
    return "BLOCKED";
  }
  ...
  if (
    report.summary.incompleteSubjectCount >
      0
  ) {
    return "PARTIAL";
  }
...
function buildReportCardScalarData({
  ...
  const calculationStatus =
    resolveStudentCalculationStatus(
      report,
    );
...
async function loadGenerationConfiguration(
  ...
  const configuration =
    await prisma.academicWeighting.findUnique({
      where: {
        id:
          weightingId,
      },
...
export async function generateClassReportCards(
  ...
  const validation =
    await validateReportCardGeneration({
      classId:
        input.classId,
  ...
  if (!validation.ready) {
    ...
    throw new Error(
      message ||
        "Report-card generation is blocked.",
    );
  }
  ...
  !validation.weighting
      ...
      throw new Error(
        "The report-card generation configuration is incomplete.",
      );
    }
    ...
    */
    ...
    const validatedWeighting =
      validation.weighting;

    const configuration =
      await loadGenerationConfiguration(
  ...
  if (
    ...
    throw new Error(
      "No student reports were produced by the academic engine.",
    );
  }
